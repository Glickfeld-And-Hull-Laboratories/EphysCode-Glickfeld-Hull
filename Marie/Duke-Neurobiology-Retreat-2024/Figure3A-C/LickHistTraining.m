...
figure
hold on
shadedErrorBar2(N_naive(1).RunEdges(1:end), nanmean(cell2mat({N_naive.RunMean}), 2), nanstd(cell2mat({N_naive.RunMean}), 0, 2)/sqrt(size(cell2mat({N_naive.RunMean}), 2)), 'LineProp', {C(4,:)});
shadedErrorBar2(N_hab(1).RunEdges(1:end), nanmean(cell2mat({N_hab.RunMean}), 2), nanstd(cell2mat({N_hab.RunMean}), 0, 2)/sqrt(size(cell2mat({N_hab.RunMean}), 2)), 'LineProp', {C(1,:)});
shadedErrorBar2(N_expert(1).RunEdges(1:end), nanmean(cell2mat({N_expert.RunMean}), 2), nanstd(cell2mat({N_expert.RunMean}), 0, 2)/sqrt(size(cell2mat({N_expert.RunMean}), 2)), 'LineProp', {C(2,:)});
xline(0, 'Color', C(6,:), 'LineStyle', '-', 'LineWidth', 1);
xline(-.68, 'Color', C(5,:), 'LineStyle', '--', 'LineWidth', 1);
xlabel('time from rewarded solenoid');
ylabel('licks/s');
...
ylabel('m/s');
 ...
 FigureWrap(NaN, 'RunPsthTraining', NaN, NaN, NaN, NaN, 2.5, 3.5);
