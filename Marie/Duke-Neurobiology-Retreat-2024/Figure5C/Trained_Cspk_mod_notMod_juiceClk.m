% Find populations of Cspks that are responsive or not to solenoid (lines
% 6- 98) in the trained
...
xline(0, 'c', 'LineWidth', 1);
xlim([-1 0.5])
%legend({'modulated by unexpected reward'; 'unmodulated by  unexpected reward'; 'reward'}, 'Location', 'northoutside', 'FontSize', 12);
%legend('boxoff')
ylim([0 10]);
xlabel('time from rewarded solenoid (s)');
ylabel('Cspk/s')
...
xline(0, 'c', 'LineWidth', 1);
xlim([-1 0.5])
%legend({'unmodulated by reward'; 'modulated by unexpected reward'; 'cue'; 'reward'}, 'Location', 'northoutside', 'FontSize', 12);
%legend('boxoff')
ylim([0 10]);
xlabel('time from rewarded solenoid (s)');
ylabel('Cspk/s')
 FigureWrap(NaN, 'Trained_Cspk_mod_notMod_juiceClk', NaN, NaN, NaN, NaN, 5, 3.5);
...
figure
h = pie([counterTrainedResp/total counterTrainedNoMod/total], {[num2str(round(100*counterTrainedResp/total)) '% Resp. unexpected reward']; 'No resp. unexepected reward'});
patchHand = findobj(h, 'Type', 'Patch'); 
patchHand(1).FaceColor = C(2,:);
patchHand(2).FaceColor = C(9,:);
title('Trained')
 FigureWrap(NaN, 'Trained_Cspk_mod_notMod_juiceClk_Pie', NaN, NaN, NaN, NaN, 1.0, 1.0);
