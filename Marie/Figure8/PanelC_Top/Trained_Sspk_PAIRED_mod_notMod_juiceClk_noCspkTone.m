...
for n = 1:length(SS_paired_ModJuiceClk)
    ...
    if ~isempty(trigger)
        trigger = [trigger.JuiceTime];
         [TrialsSpike, trigger] = TrialsWith_outCS(trigger.', CS_paired_ModJuiceClk, n, [-.62 -.4]);
...
for n = 1:length(SS_paired_NOmodJuiceClk)
    ...
    if ~isempty(trigger)
        trigger = [trigger.JuiceTime];
                 [TrialsSpike, trigger] = TrialsWith_outCS(trigger.', CS_paired_NOmodJuiceClk, n, [-.62 -.4]);
...
xline(-.62, 'k');
xline(-.4, 'k');
FigureWrap(NaN, 'Trained_Sspk_PAIRED_mod_notMod_juiceClk_noCspkTone', NaN, NaN, NaN, NaN,  2.5, 1.5);
