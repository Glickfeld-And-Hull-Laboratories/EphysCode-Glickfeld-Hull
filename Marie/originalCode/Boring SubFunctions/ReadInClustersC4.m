function [NoiseUnits, GoodUnits, MUnits, GoodANDmua] = ReadInClustersC4(cluster_struct)
...
for k=1:length(cluster_struct) % make vectors of units that are good, noise, or mua
    ...
    if strcmp(cluster_struct(k).group,'mua')
        MUnits(m,1) = (cluster_struct(k).cluster);
        m = m+1;
    end
    if (strcmp(cluster_struct(k).group,'good')) | (strcmp(cluster_struct(k).group,'mua'))
        GoodANDmua(gm,1) = (cluster_struct(k).cluster);
        gm = gm +1;
    end
