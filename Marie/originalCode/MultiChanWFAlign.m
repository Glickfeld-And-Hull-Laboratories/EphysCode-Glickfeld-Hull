function [time, MainWaveforms, MultiChanWFStruct, Scale, testerShiftTime] = MultiChanWFAlign(struct, dataLength, n, timeLim, TimeGridA, TimeGridB, unit, map, color, PlotSamps, PlotMean, ScalePass)
...
TSset = timestamps(TSindex); %set timestamps we will sample on every channel (unaligned)

figure
if ~isnan(TimeGridA)
    [time, MainWaveforms, ~] = SampleWaveformsTimeLimTGzerogo(TimeGridA, TimeGridB, struct, dataLength, n, timeLim, unit, CenterChan, PlotSamps); %first we want to get size of unit so we can scale
else 
...
TSset = TSset + shiftTime; %align timestamps
testerShiftTime = TSset;
figure

NewStruct=struct(index);
NewStruct.timestamps = TSset;
figure
[time2, Waveforms2, SampleTS2] = SampleWaveformsTimeLimTGsetTS(TimeGridA, TimeGridB, NewStruct, .012, 100, [0 inf], 415, 84, TSset);
saveas(gca, 'tester')
close gcf
...
Waveforms = zeros(nSamp, length(TSset));
for i = 1:length(TSset)
samp0 = TSset(i,1)*SampRate(meta);
samp0 = int64(samp0);
SampleTS(i) = TSset(i,1);
