% =============================================================
%  from DemoReadSGLXData.m from SpikeGLX documentation
%
...
function [time, LFPstruct] = LFPextract(TimeGridA, TimeGridB, dataLength, timeLim, chanspan, Trigger, color, plotALL)
...
SampleTS = zeros(length(Trigger),1);
...
if ~isempty(timestamps)
    LFP = zeros(nSamp, length(TSindex));
...
plot(time, AvgLFP, 'color', color);
hold off
else
    LFP = 0;
end
LFPstruct(c).chan = chan;
LFPstruct(c).LFP =LFP;
LFPstruct(c).Avg = AvgLFP;
LFPstruct(c).timeLim = timeLim;
