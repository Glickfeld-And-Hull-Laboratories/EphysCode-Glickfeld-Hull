function [time, MainWaveforms, MultiChanWFStruct, Scale] = MultiChanWF_morechans(struct, dataLength, catchTime, n, timeLim, TimeGridA, TimeGridB, unit, map, color, PlotSamps, PlotMean, ScalePass)
...
if step == 0
   ...
   % f = warndlg('No spikes in WF mean calculation window. AvgWF = 0');
    ...
    counter = 0;
for j = (CenterChan - 18):(CenterChan + 18)%for each chan we want to examine
%if ~isempty(timestamps)
    if j < 0
        continue
    end
