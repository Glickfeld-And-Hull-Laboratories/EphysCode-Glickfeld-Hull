...
function [N, edges] = xCorrStructNewLimitsLineINDEX_TGmasterOnly(TimeGridA, TimeGridB, struct, xmin, xmax, bwidth, unit1, unit2, limMin, limMax, color, plotBoo, SD, SDboo) %times in seconds, n unit of interest for struct (.unitID= string, .timestamps= vector 
...
[N, ~] = FreqLine(N, edges, length(Unit1), color, NaN, plotBoo);
if ~SDboo == 0
