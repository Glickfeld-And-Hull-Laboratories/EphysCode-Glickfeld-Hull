...
function [time, Waveforms, SampleTS] = SampleWaveformsTimeBlocks(TimeGridA, TimeGridB, struct, dataLength, n, timeLim, color, unit, chan)
...
if length(timestamps) < n
    disp('there are not enough spikes to calculate the desired average. Average will be calculated with all available spikes')
    n = length(timestamps)
end
...
for i = 1:n
  ...
  randn = randi([(((i-1)*step)+1),(i*step)]);
        ...
        display = ['average first ' n 'waveforms of ' length(timestamps)];
        disp(display)
        disp(n)
