function [FR, WF, TGlim2WFs, struct2] = CellCharWorkupDrugStruct_uhdJulien(unitIN, struct, trigger, TimeGridA, TimeGridB, xmin, xmax, TrigMin, TrigMax, TimeLim1, TimeLim2, DrugLinesStruct, ISIxaxis, binsize, color, nametag, map)
...
hold on
for n = 1:size([struct(unitIN).WF_samples.TimeLim1], 2)
    plot(struct(unitIN).WF_samples.time, struct(unitIN).WF_samples.TimeLim1(:,n), 'k')
end
title('Non-stim WF')
ylabel('V');
xlabel('msec');
...
plot(struct(unitIN).WFmean.time, struct(unitIN).WFmean.Laser, 'b')
plot(struct(unitIN).WFmean.time, struct(unitIN).WFmean.TimeLim2, 'm')
 plot(struct(unitIN).WFmean.time, struct(unitIN).WFmean.TimeLim1, 'r')
hold off
title('DrugTG M, StimPre B, ')
ylabel('V');
xlabel('msec');
hold off
if ~isnan(ylimitWF2)
...
if MultiChanWFboo == 1
    nexttile([2 2]);
    hold on
    [time, WF, TGlim1WFs, Scale] = MultiChanWF_uhd(struct, .003, .001, 100, TimeLim1, TimeGridA, TimeGridB, unit, map, 'k', 0, 0, NaN);
    lim1wf = 1
    if DrugOverlayMulti == 1
        if ~isnan(TimeLim2)
            [~, ~, TGlim2WFs, ~] = MultiChanWF_uhd(struct, .003, .001, 100, TimeLim2, TimeGridA, TimeGridB, unit, map, color2, 0, 0, Scale);
        end
        lim2wf = 1
    end
    if laserStimOverlayMulti == 1
        [~, ~, StimWFs, ~] = MultiChanWF_uhd(struct, .003, .001, 100, TimeLim1, TimeGridB, TimeGridB+StimTimeWin, unit, map, 'b', 0, 0, Scale);
    end
    stim_WF=1
    hold on
    for n = 1:length(TGlim1WFs)
        if (max(TGlim1WFs(n).AvgWf) - min(TGlim1WFs(n).AvgWf)) > .1*Scale
...
hold off
struct2(unitIN).TGlim1WF = TGlim1WFs;
struct2(unitIN).TGlim2WFs = TGlim2WFs;
struct2(unitIN).StimWFs = StimWFs;
end
...
if HeatMapBoo == 1
nexttile
h =  heatmap([0:1:15], [23:-1:0], (fliplr(struct(unitIN).Heats)).');
h.Colormap = parula;
h.GridVisible = 'off';
