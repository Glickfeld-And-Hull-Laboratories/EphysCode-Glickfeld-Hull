function ks2_working_toBinary( rez)
...
% 2nd important note:
% Make sure you set the correct version of kilosort. Known versions are
% '2.0' and '2.5' and '3.0'
KSver = '2.5'
...
%load( rezFullPath ); commented out 1/6/21 MEH
[binPath, binName, binExt] = fileparts(rez.ops.fbinary)
tPos = strfind(binName, '.imec');
temp = extractBetween(binName,1,tPos-5);
baseName = temp{1};
suffix = extractAfter(binName,'tcat');
...
%copyfile( modelMetaFullPath, fullfile(binPath, metaName) );
...
% After filtering, the data is whitened and then transposed bacy to NChan
...
elseif strcmp(KSver,'2.5') || strcmp(KSver,'3.0')
    ...
    for ibatch = 1:ops.Nbatch
        ...
        % if skipping unwhiteningrescale by the average of the diagonal of the whitening
        % matrix. This will keep the voltage values in range during the 2nd
        % sort.
        %norm = mean(diag(rez.Wrot));
        %dat = int16(single(dat)./norm);
...
if strlength(modelMetaFullPath) > 0
    ...
    fmeta = fopen( fullfile(binPath, metaName), 'w');
