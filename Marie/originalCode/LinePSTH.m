...
function  [N, edges] = LinePSTH(TS1, n, struct, xmin, xmax, bwidth, timeLim, color, FaceAlpha)
normboo = 1;
stdevboo = 0;
...
for  i=1:L1                                           % for every element in the first spiketime vector (trigger)
    ...
    for j = 1:L2                                 % for every element in the second (or mirror) spiketime vector (spike timestamp)
       test = TS2(j,:)- TS1(i,:);                  % get difference between spike and trigger
       if test == 0
...
[N, ~] = FreqHist(N, edges, length(TS1), color, FaceAlpha, 0); % because we are counting through the trials with the spiketimes, not the triggers
if normboo == 0
    N = N/N(1);
end
plot(edges(1:end-1), N, color)
%length(TS1)
[meanLine, stdevLine] = StDevLine(N, edges, -.53);
%AddStDevLines(meanLine, stdevLine);
if stdevboo == 1
yline((meanLine + 2*stdevLine), 'g', 'LineWidth', 2);
if (meanLine - 2*stdevLine) >0
