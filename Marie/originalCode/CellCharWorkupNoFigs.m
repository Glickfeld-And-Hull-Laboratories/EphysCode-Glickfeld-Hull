function [FR, WF, wfStructStruct] = CellCharWorkupNoFigs(unit, struct, trigger, TimeGridA, TimeGridB, xmin, xmax, TrigMin, TrigMax, TimeLimBase, TimeLim1, TimeLim2, ISIxaxis, binsize, color, nametag, JuiceTimes, LickTimes, RTs, JuiceBin, map)
...
DrugLines = 1;
   ...
   DrugTime3 = TimeLim2(2); %This really should be names DrugTimeSec3
    ...
    DrugName1 = 'ex blockers';
    DrugName2 = 'early block end';
    DrugName3 = '';
    DrugName4 = 'block effect start';
    TrialName5 = 'block effect end';
...
if psth ==  1
nexttile
OneUnitHistStructTimeLim(trigger, unit, struct, TrigMin, TrigMax, binsize, TimeLim1, 4, color, 1);
hold on
if doublechart == 1
    OneUnitHistStructTimeLim(trigger, unit, struct, TrigMin, TrigMax, binsize, TimeLim2, 4, color2, .7);
end
title('Resp To Laser');
ylabel('Hz');
xlabel('msec');
