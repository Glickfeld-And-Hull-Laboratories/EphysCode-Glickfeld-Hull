...
function [LatLow, LatHigh, modBoo, Dir, doubleBoo] = LatencyMod(StDevLim, N, edges, SD, AccRespWindow, plotBoo)
...
if ~isempty(crossingsLow)
    crossingsLow = crossingsLow(crossingsLow >= minRespLat);
    crossingsLow = crossingsLow(crossingsLow <= maxRespLat);
    if ~isempty(crossingsLow)
...
if ~isempty(crossingsHigh)
    crossingsHigh = crossingsHigh(crossingsHigh >= minRespLat);
    crossingsHigh = crossingsHigh(crossingsHigh <= maxRespLat);
    if ~isempty(crossingsHigh)
