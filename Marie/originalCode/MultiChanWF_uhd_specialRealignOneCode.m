function [time, MainWaveforms, MultiChanWFStruct, Scale] = MultiChanWF_uhd_specialRealignOneCode(struct, dataLength, catchTime, n, timeLim, Chan, TimeGridA, TimeGridB, unit, map, color, PlotSamps, PlotMean, ScalePass)
...
%to run this OneCode sucessfully, you need to go to and return the
%un-aligned waveforms, instead of aligned.

index = find([struct.unitID] == unit) %% n changes to index in struct pointing to specified unit
%CenterChan = struct(index).channel;
CenterChan = Chan %in this version I give it a channel to focus on, not the peak channel. We are looking for granule cells.


figure;
if ~isnan(TimeGridA)
    [time, MainWaveforms, ~] = SampleWaveformsTimeLimTGzerogo(TimeGridA, TimeGridB, struct, dataLength, n, timeLim, unit, CenterChan, 1); %first we want to get size of unit so we can scale
else 
    [time, MainWaveforms, ~] = SampleWaveformsTimeLimNewzerogo(struct, dataLength, n, timeLim, unit, CenterChan, 1); %first we want to get size of unit so we can scale
end
close gcf;
...
timestamps = (timestamps - catchTime); % catch the begining of the waveform
...
if step == 0
   ...
   % f = warndlg('No spikes in WF mean calculation window. AvgWF = 0');
    ...
    counter = 0;
for j = (CenterChan - 0):(CenterChan + 0)%for each chan we want to examine
%if ~isempty(timestamps)
    if j < 0
        continue
    end
...
%modified from AlignWaveformsXcorr
sampRate = 30000;
...
shiftmax = 30;
%min-centered
[Thresh, ~] = min(AVG(30:70)); %introduces a threshold to be included as a trace with a possible Grn cell spike- min is lower than the average negative after wave
figure
hold on
counter2 = 1;
 for n = 1:size(Waveforms,2)
[minY,I] = min(Waveforms(30:70, n));
if minY < Thresh
     WaveformsMinAligned(:, counter2) = zeros(165, 1);
%shift = I-30
plot(Waveforms(:,n));
scatter(30+I-1, Waveforms(30+I-1, n));
     WaveformsMinAligned(42-I:131-I,counter2) = Waveforms(:,n);
%      scatter(60+shift, WaveformsMinAligned(60+shift,n));
counter2 = counter2 + 1;
end
 end
 ...
 figure
 hold on
 for n = 1:size(WaveformsMinAligned,2)
     plot(WaveformsMinAligned(:,n));
     scatter(30, WaveformsMinAligned(30,n));
 end
 Waveforms = WaveformsMinAligned;
...
if PlotSamps == 1
    hold on
%plot(time+X/1000, Waveforms+Y*Scale/10, 'Color', [.5 .5 .5]);
plot( Waveforms+Y*Scale/10, 'Color', [.5 .5 .5]);
end
if PlotMean == 1
    hold on
%plot(time+X/1000, AvgWvF+Y*Scale/10, color);
plot(Waveforms+Y*Scale/10, 'Color', [.5 .5 .5]);
...
%textx = time(end/3) + X/1000;
%texty = max(AvgWvF + Y*Scale/1)+100*(std(std(Waveforms)));
...
if PlotMean == 1
    ...
    X = map(jIndex).xcoord - map(MapIndexCenter).xcoord;
    Y = map(jIndex).ycoord - map(MapIndexCenter).ycoord;
%plot(time+X/1000, BiggestWF+Y*Scale/10, color, 'LineWidth', 2);
plot(BiggestWF+Y*Scale/10, color, 'LineWidth', 2);
end

%plot([-.008, -.005], [-.0008, -.0008], 'k', 'LineWidth', 1);
%plot([-.008, -.008], [-.0008, -.0007], 'k', 'LineWidth', 1);
if isnan(ScalePass)  %assuming Scale would have been passed in if this was the first time plotting this data and we would want to print the channel info
for m = 1:length(MultiChanWFStruct)
X = MultiChanWFStruct(m).X;
Y = MultiChanWFStruct(m).Y;
textx = time(end/3) + X/1000;
texty = max(AvgWvF + Y*Scale/10)+10*(std(std(Waveforms)));
text(textx, texty, num2str(MultiChanWFStruct(m).Chan), 'FontSize', 6); 
end
f = gca;
Xzero = f.XLim(1);
Yzero = f.YLim(1);
