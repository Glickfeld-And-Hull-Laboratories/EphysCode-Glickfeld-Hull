function [FR, WF, wfStructStruct] = CellCharWorkupDrug_noLaser(unit, struct, trigger, TimeGridA, TimeGridB, xmin, xmax, TrigMin, TrigMax, TimeLim1, TimeLim2, DrugLinesStruct, ISIxaxis, binsize, color, nametag, map)
...
if ~isnan(TimeLim2)
title_ = [unitStr ' fires at ' FRstr ' on channel ' channelStr ' baseline [' TimeLimStart ' ' TimeLimEnd '] drugComp [' num2str(TimeLim2(1)) ' ' num2str(TimeLim2(2)) '] in ' struct(unitIN).layer ', ' num2str(struct(unitIN).PC_dist) ' um from PC ' nametag];
else
...
set(gcf,'Position',[0 800 2000 400]);
layout1 = tiledlayout(1,6);
%fprintf('fig?')
...
%[~, ~, Lim2Wfs, wfSampSize, ~] = MultiChanWF(struct, .003, 100, TimeLim2, NaN, NaN, unit, map, 'g', 0, 1, Scale);
 ...
 if laserStimOverlayMulti == 1
     [~, ~, StimWFs, ~] = MultiChanWF(struct, .003, .001, 100, TimeLim1, TimeGridB, TimeGridB+StimTimeWin, unit, map, 'b', 0, 1, Scale);
     hold on
...
hold on
if doublechart == 1
    autoCorrStructNewLimitsTG(TimeGridA, TimeGridB, struct, xmin, xmax, binsize, unit, TimeLim2, color2, 1);
end 
title('Baseline ACG');
ylabel('Hz');
xlabel('msec');
