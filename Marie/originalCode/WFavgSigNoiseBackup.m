...
function [SigNoise2, AvgWvF, StDev] = WFavgSigNoise(TimeGridA, TimeGridB, struct, AllBinary, dataLength, timeLim, unit)
...
%nChan = nChan 
%ch = chan;
%AllBinaryGainCorrect = GainCorrectIM(AllBinary, [ch], meta); %remove for speed if WholeRecord ==1

%figure
%plot(dataArray(chan,:))
%sizeDataArray = size(dataArray)
%sizeAllBinary = size(AllBinary)
% % % change y values to volts
%        For an analog channel: gain correct saved channel ch (1-based for MATLAB).
%ch = chan;
...
%step = floor(length(timestamps)/m);

%timestamps = (timestamps - .001); % catch the begining of the waveform
%%this is already done here
%nSamp = floor(dataLength * SampRate(meta));
%nSamp = int64(nSamp);
%SampleWF = zeros(nSamp, n);
%timestep = 1/SampRate(meta);
%time = 0:timestep:dataLength-1/SampRate(meta);
%SampleTS = zeros(n,1);
...
%    n = 10000;
%for i = 1:m
  %randm(i) = randi([(((i-1)*step)+1),(i*step)]);
  
%samp0 = timestamps(randm)*SampRate(meta);
%samp0 = int64(samp0);
%SampleTS(i) = timestamps(randm);
%dataArray = ReadBin(samp0, nSamp, meta, binName, path);
%dataArray = AllBinary(:,samp0:(nSamp + samp0-1));
% % % change y values to volts
% For an analog channel: gain correct saved channel ch (1-based for MATLAB).
%ch = chan;
%
