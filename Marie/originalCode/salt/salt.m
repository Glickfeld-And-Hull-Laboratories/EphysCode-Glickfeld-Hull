...
nmbn = floor(wn/dt);   % number of bins for latency histograms; windowSize/timeResolution, aka window size in bins %%changed 'round' to 'floor'
edges = 0:nmbn+1;  % bin boundaries; time resolution bins in a window (+1? for right extra edge?)
nm = floor(st/nmbn);   % size of the null dist. sample; number of windows
[lsi slsi] = deal(zeros(tno,nm));   % preallocate latency matrices; #trial rows, data points/bin colums (preallocate lsi and slsi)
[hlsi nhlsi] = deal(zeros(nmbn+1,nm));    % preallocate latency histograms #number of edges rows, data points/bin columns
next = 1;   % counter
for t = 1:nmbn:(st)   % loop through baseline windows % Can change "st" to "st-nmbn" to account for non-divisible baseline into discretized collection window if needed, I think
...
% -------------------------------------------------------------------------
function [p_value Idiff] = makep(kld,kn) %kld = jsd
