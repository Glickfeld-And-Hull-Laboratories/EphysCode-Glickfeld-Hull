...
function [time, Waveforms, SampleTS] = SampleWaveformsIfRun(ifrun, struct, dataLength, n, unit, chan)
title1 = ['Unit ' num2str(unit)];
title2 = ['chan ' num2str(chan)];
title_ = [title1 ' on ' title2];
...
TS2 = timestamps; % just rename timestamps
...
for i = 1:length(ifrun)     % make two timestamp vectors, one from the central portion of running trial and one from the central portion of no-run trials
 ...
 if length(TSstop) < n
     fprintf('There are not enough spikes during NoRun \n')
 end
 if length(TSrun) < n
     fprintf('There are not enough spikes during Running \n')
 end
...
for m =1:2
    if m == 1
        timestamps = TSstop; %The first time we use TSstop
    end
    if m == 2
        timestamps = TSrun;
    end
...
timestamps = (timestamps - .003); % catch the begining of the waveform
...
hold on
for p = 1:n
    plot(time, Waveforms(:,p), 'k');
end
plot(time, AvgWvF, 'r');


if m ==1
    title_noRun = [title_ ' No Run'];
    title(title_noRun);
end
if m == 2
    title_Run = [title_ ' Running'];
    title(title_Run);
end
...
figure
plot(time, RunningWvF, 'g');
hold on
plot(time, noRunWvF, 'r');
titleBoth = [title_ ' Run/NoRun'];
title(titleBoth);
hold off
