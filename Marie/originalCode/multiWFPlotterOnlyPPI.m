function BiggestWFstruct = multiWFPlotterOnlyPPI(structstruct, string, figboo, chan)
figure
hold on
...
for m = 1:length([structstruct.WFstruct])%length(structstruct)
    ...
    BiggestWFstruct(m).SizeReBase = Sizer/Base;
     ...
     % [EarlyPeak, Peakloc] = max(struct(n).AvgWf(1:30));
    %Peakloc = 29;
    %[AHP, AHPloc] = min(struct(n).AvgWf(35:50));
    %AHPloc = AHPloc + 34;
