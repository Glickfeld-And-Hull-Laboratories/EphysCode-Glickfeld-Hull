function RasterMatrix = OrganizeRasterEvents(NewTrig, event, xmin, xmax, color2)

title_ = num2str(event);
...
for m = 1:stop
    clear k;
    n = [NaN];
    lickWin = ((event > (NewTrig(m)- xmin)) & (event < (NewTrig(m)+ xmax)));
