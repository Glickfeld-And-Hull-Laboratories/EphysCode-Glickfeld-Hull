...
% much easier!
%
function [TraceData, p] = LongTraceRead_forUHDvis(TimeWindow, unit, struct, chansfromChan)
% TimeWindow = [tmin tmax] to extract
% n = how many waveforms to extract
% timestamps = when unit is firing (unitxxx)
% chan = channel in SPIKEGLX NOTATION- zero-based
% to add another unit, type 'hold on' and use AddUnitToTrace, eg AddUnitToTrace ([20 20.2], unit499, 138, 'r');

% Ask user for binary file
% Ask user for binary file
unitIN = find([struct.unitID] == unit);
chan = struct(unitIN).channel + chansfromChan;
timestamps = extractUnitTimestamps(struct, unit); %turn unit ID into list of timestamps for spikes from that unit
reporter = timestamps;
...
dataLength=(TimeWindow(2)-TimeWindow(1));

timestamps = ((timestamps)-.0005); % catch the begining of the waveform
nSamp = floor(dataLength * SampRate(meta));
nSamp = int64(nSamp);
TraceData = zeros(nSamp,1);




 
SampleTS = TimeWindow(1); 
samp0 = TimeWindow(1)*SampRate(meta);    
samp0 = int64(samp0);
...
TraceData(:,1) = dataArray(chan,:);
t = 1:1:((length(TraceData(:,1))));
t = t/SampRate(meta) + SampleTS;
t = t.';


  
%figure
p = plot(t, TraceData(:,1), 'k');
p.DataTipTemplate.DataTipRows(1).Format = '%5.6f';
%plot(Waveforms(:,i));
xlim([t(1) t(end)]);
hold on

TSwindow = (timestamps(( timestamps >= TimeWindow(1) & (timestamps <=TimeWindow(2)))));
mininSamp = floor(.001*SampRate(meta)); % .001 is size of unit trace
...
x = miniTS;
    ...
    for m = 1:(length(TSwindow)-1)
    miniTS = [miniTS x];
    end


for k = 1:length(TSwindow)
    %figure
samp0 = int64(TSwindow(k)*SampRate(meta));
...
% (1-based). For imec data there is never more than one saved digital word.
  ...
  % % % 
miniWaveforms(:,k) = dataArray1(chan,:);

miniTS(:,k) = miniTS(:,k) + (TSwindow(k)- TimeWindow(1))*SampRate(meta);
