function [FR, WF, wfStructStruct] = WFforDepressionSpont(unit, struct, trigger, TimeGridA, TimeGridB, timeLim, MaxSpikeNum, DrugLinesStruct,  color, nametag, map)
...
for s = 1:length(TSmatrix) 
       %would be way more efficient to make this matrix one time in the function above and pass it in....
        latcheck = 1;
        if SpikeNumber ==1
            if  ((TSmatrix(s).ts(2) - TSmatrix(s).ts(1)) > .0051 || (TSmatrix(s).ts(2) - TSmatrix(s).ts(1)) <=0)
            latcheck = 0;
            end
        end
