...
function [N, edges] = xCorr3DNewLimitsLine(struct, xmin, xmax, bwidth, unit1, unit2, limMin, limMax, color, SD, SDboo) %times in seconds, n unit of interest for struct (.unitID= string, .timestamps= vector 
...
for n = 1:length(FRbins)-1
ThreeDstruct(n).LowFR = FRbins(n);
ThreeDstruct(n).HighFR = FRbins(n+1);
ThreeDstruct(n).k = 1;  
end
ThreeDstruct(n+1).LowFR = FRbins(n+1);
ThreeDstruct(n+1).HighFR = inf;
ThreeDstruct(n+1).k = 1;  
...
for j = 1:(L1)-1                                           % for every element in the first spiketime vector
preIntervalFR = 1/(Unit1(j+1) - Unit2(j));
FRbinIndex = find(([ThreeDstruct.HighFR] > round(preIntervalFR)), 1, 'first');
    for i=1:L2-1                                   % for every element in the second (or mirror) spiketime vector
...
if length(ThreeDstruct(n).useDeltas) > 50
...
[N, ~] = FreqLine(N, edges, length(Unit1), ParulaColors(n,:), num2str(FRbins(n)), 1);
if ~SDboo == 0
...
%tiledlayout(flow);
 ...
 for n = 1:length(ThreeDstruct)
tester(n) = length(ThreeDstruct(n).useDeltas);
end
figure
plot([ThreeDstruct.LowFR],tester)
