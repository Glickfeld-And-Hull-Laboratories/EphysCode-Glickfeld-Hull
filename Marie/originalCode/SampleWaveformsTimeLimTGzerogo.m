...
function [time, AlignedWaveforms, SampleTS] = SampleWaveformsTimeLimTGzerogo(TimeGridA, TimeGridB, struct, dataLength, n, timeLim, unit, chan, plotALL)
...
size(Waveforms)
size(AvgWvF)
[AlignedWaveforms, shiftTime, peakcentered, noshift] = AlignWaveformsXcorr(Waveforms, 10);
...
hold on
% size(time)
% size(AlignedWaveforms)
if plotALL == 1
 for p = 1:size(AlignedWaveforms, 2)
     plot(time, AlignedWaveforms(:,p), 'k');
 end
end
plot(time, AlignedAverage, 'r');
else
    AlignedWaveforms = 0;
end
