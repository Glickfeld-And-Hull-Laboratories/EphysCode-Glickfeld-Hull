function [FR, WF, wfStructStruct] = WFforDepressionSpont_isicalc(unit, struct, trigger, TimeGridA, TimeGridB, timeLim, MaxSpikeNum, DrugLinesStruct,  color, nametag, map)
...
FR = FRstructTimeGridTimeLimit(TimeGridA, TimeGridB, timeLim, struct, unit, color, 0);
...
for s = 1:(length(timestamps)-MaxSpikeNum)
 TSmatrix(s).ts = timestamps(s:s+MaxSpikeNum); 
end
...
if MultiChanWFboo == 1
    ...
    for SpikeNumber = 1:MaxSpikeNum
        SpikeNumber = SpikeNumber
         if SpikeNumber == 1
            [time, WF, TGlim1WFs, Scale] = MultiChanWFforDepression(struct, .003, 100, TSmatrix, timeLim, SpikeNumber, MaxSpikeNum, TimeGridA, TimeGridB, unit, map, 'k', 0, 1, NaN);
         end
         if SpikeNumber >1
             [time, WF, TGlim1WFs, ~] = MultiChanWFforDepression(struct, .003, 100, TSmatrix, timeLim, SpikeNumber, MaxSpikeNum, TimeGridA, TimeGridB, unit, map, 'k', 0, 1, Scale);
         end
...
function [time, MainWaveforms, MultiChanWFStruct, Scale] = MultiChanWFforDepression(struct, dataLength, n, TSmatrix, timeLim, SpikeNumber, MaxSpikeNum, TimeGridA, TimeGridB, unit, map, color, PlotSamps, PlotMean, ScalePass)
...
counter = 1;
for s = 1:length(TSmatrix) 
       %would be way more efficient to make this matrix one time in the function above and pass it in....
        latcheck = 1;
        if SpikeNumber ==1
            if  (TSmatrix(s).ts(2) - TSmatrix(s).ts(1) > .0051) %& TSmatrix(s).ts(2) - TSmatrix(s).ts(1) < .0101)
            latcheck = 0;
            end
        end
        for v = 2:SpikeNumber %MaxSpikeNum
        if  (TSmatrix(s).ts(v) - TSmatrix(s).ts(v-1) > .0051) %& TSmatrix(s).ts(2) - TSmatrix(s).ts(1) < .0101)
            latcheck = 0;
            break
        end
        end
