function [FR, WF, wfStructStruct] = CellCharWorkupDrugStruct_uhd_noLaser(unit, struct, trigger, TimeGridA, TimeGridB, xmin, xmax, TrigMin, TrigMax, TimeLim1, TimeLim2, DrugLinesStruct, ISIxaxis, binsize, color, nametag, map)
...
trigger(1) = struct(unitIN).timestamps(round(end/2)); %point for raw data trace to use.
if ~isnan(TimeLim2)
...
if extraHist == 1
nexttile
OneUnitHistStructTimeLim(trigger, unit, struct, TrigMin, TrigMax, binsize, TimeLim1, 4, color, .5, 1);
hold on
if doublechart == 1
...
%plot(trigger);
trial = 1; %which laser stimulation trial to use for the trace
