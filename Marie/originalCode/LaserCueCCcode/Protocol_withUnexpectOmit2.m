...
for n = 1:round(trials/20)
    ...
    for trial = 1:trials
        ...
        if ~ismember(rUnexp)  && ~ismember(rOmit)   %if trial is not unexpected trial
            for n = 1:13
        ...
        if ismember(rUnexp) %if trial is unexpected trial
        ...
        if ismember(rOmit) %if trial is unexpected trial
            for n = 1:13
                writePWMVoltage(a, 'D11', 0);
                pause(.011);
                writePWMVoltage(a, 'D11', 5);
