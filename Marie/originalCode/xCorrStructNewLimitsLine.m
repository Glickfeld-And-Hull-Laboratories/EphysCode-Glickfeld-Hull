...
function [N, edges] = xCorrStructNewLimitsLine(struct, xmin, xmax, bwidth, unit1, unit2, limMin, limMax, color, SD, SDboo) %times in seconds, n unit of interest for struct (.unitID= string, .timestamps= vector 
...
[N, ~] = FreqLine(N, edges, length(Unit1), color, NaN, 1);
if ~SDboo == 0
