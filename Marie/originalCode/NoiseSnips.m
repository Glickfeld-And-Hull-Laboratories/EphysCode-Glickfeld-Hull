function [BinaryStruct, NoiseMetaData] = NoiseSnips(TimeLim, numSnips, lengthSnips)
%calculate noise on every channel for the number and lengths of snips
%specified.
...
%chan = chan +1;
    ...
    EndSamp = SampRate(meta)*SpikeTimeSec(end)
    else
        EndSamp = SampRate(meta)*TimeLim(2);
    end
    
    %samp0 = SpikeTimeSec(1)*SampRate(meta);
    FirstSamp = TimeLim(1)*SampRate(meta);
    FirstSamp = int64(FirstSamp);
    SnipSamp = lengthSnips * SampRate(meta);
    step = floor(((EndSamp/SampRate(meta))-TimeLim(1))/numSnips); %Size steps to take to choose snip samples in seconds
        if step <1
            if lengthSnips <1
                step = 1;
            else
                error('Too many samples for this length sample')
            end
        end
        if lengthSnips > step
            error('mismatch between number of samples and sample length.')
        end
                fprintf(['step is ' num2str(step) '\n']);
    nSamp = int64(SnipSamp); %nsamp is size of data you want read
    AllBinary = [];
    
    for n = 1:numSnips
         fclose('all');
    samp0 = (n-1)*step*SampRate(meta); 
    samp0 = int64(samp0);
        fprintf(['start at ' num2str((n-1)*step) 'sec\n']);
        fprintf(['nSamp is ' num2str(nSamp) '\n\n']);
   TempBinary = ReadBin(samp0, nSamp, meta, binName, path);
   AllBinary = [AllBinary TempBinary];
   fclose('all');
    end
     ...
     chans = OriginalChans(meta);
      C = textscan(meta.imroTbl, '(%*s %*s %*s %d %d %*s', ...
        ...
        APgain = double(cell2mat(C(1)));
         nAP = length(APgain);
        nNu = nAP * 2;  %I have no idea
        
    % Common conversion factor
    fI2V = Int2Volts(meta);
    chanList = [0:383];
    
     for i = 1:length(chanList) %chanlist aka [ch]
        j = chanList(i);    % index into timepoint
        k = chans(i);       % acquisition index %CHANGED from J to I 1/26/22 MEH
        if k <= nAP
            conv = fI2V / APgain(k);
        elseif k <= nNu
            conv = fI2V / LFgain(k - nAP);
            x = 'something very wrong here'
        else
            continue;
        end
        BinaryStruct(i).chan = chanList(i);
        BinaryStruct(i).Binary = AllBinary(i,:) * conv; %CHANGED from dataArray(j,:) to i.
    end
    
NoiseMetaData.File = fullfile(path, binName);
NoiseMetaData.Call =  ['NoiseSnips([' num2str(TimeLim(1)) ' ' num2str(TimeLim(2)) '], ' num2str(numSnips) ', ' num2str(lengthSnips)];
    
   
   
   
%   AllChannels = [0:384].'; % create channels per SpikeGLX
%   for c = 1:length(AllChannels)
%BinaryFiltAllchannels(c).chan = AllChannels(c);

%matlabCH = AllChannels(c)+1; %change from spikeGLX (0-based) to Matlab array Index (1-based)
%AllBinaryGainCorrect = GainCorrectIM(AllBinary, [ch], meta); %Gain Correct for Channel of intest
%BinaryFiltAllchannels(c).Binary = AllBinaryGainCorrect(matlabCH,:);
%clear AllBinaryGainCorrect;

  
end
%this is needed to get individual channel in volts    
% % % change y values to volts
% For an analog channel: gain correct saved channel ch (1-based for MATLAB).
%meta = ReadMeta(binName, path);

%chan = chan +1; %For an analog channel: gain correct saved channel ch
%(1-based for MATLAB). I think this is needed but always unsure.
...
%        AllBinary = GainCorrectIM(ForNoise, [ch], meta);
%    else
%        AllBinary = GainCorrectNI(ForNoise, [ch], meta);
%    end
    
%   OnceChannel = AllBinary(chan,:);
...
function dataArray = ReadBin(samp0, nSamp, meta, binName, path)
    ...
    %nSamp
    %nSamp-samp0 
    %fprintf(['nSamp here is ' num2str(nSamp) '\n']);
    sizeA = [nChan, nSamp];

    fid = fopen(fullfile(path, binName), 'rb');
    filespot = (samp0 * 2 * nChan);
    reporter = fseek(fid, filespot, 'bof');
    if reporter ~=0
        error('NoiseSnips line 201');
    end
