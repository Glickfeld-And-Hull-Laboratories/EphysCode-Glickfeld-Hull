function [time, MainWaveforms, MultiChanWFStruct, Scale] = MultiChanWF(struct, dataLength, catchTime, n, timeLim, TimeGridA, TimeGridB, unit, map, color, PlotSamps, PlotMean, ScalePass)
...
MultiChanWFStruct(1).BiggestWaveInfo.BiggestChan = BiggestChan;
MultiChanWFStruct(1).BiggestWaveInfo.BiggestWFmean = BiggestWF;
if ~isnan(TimeGridA)
    [time, MainWaveforms, ~] = SampleWaveformsTimeLimTGzerogo(TimeGridA, TimeGridB, struct, dataLength, n, timeLim, unit, BiggestChan, 1); %first we want to get size of unit so we can scale
else
        [time, MainWaveforms, ~] = SampleWaveformsTimeLimTGzerogo(TimeGridA, TimeGridB, struct, dataLength, n, timeLim, unit, BiggestChan, 1); %first we want to get size of unit so we can scale
    %[time, MainWaveforms, ~] = SampleWaveformsTimeLimNewzerogo(struct, dataLength, n, timeLim, unit, BiggestChan, 1); %first we want to get size of unit so we can scale
end
MultiChanWFStruct(1).BiggestWaveInfo.BiggestWFall = MainWaveforms;
if PlotMean == 1
    hold on
     jIndex = find ([map.chan] == BiggestChan); %get index of map for the channel of interest
    X = map(jIndex).xcoord - map(MapIndexCenter).xcoord;
    Y = map(jIndex).ycoord - map(MapIndexCenter).ycoord;
plot(time+X/5000, BiggestWF+Y*Scale/30, color, 'LineWidth', 2);
end
