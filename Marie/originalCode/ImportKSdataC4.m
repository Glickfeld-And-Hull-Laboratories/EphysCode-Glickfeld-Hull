...
%
%
function [cluster_struct, GoodUnits, MultiUnits, GoodANDmua, AllUnits, AllUnitStruct, GoodUnitStruct, MultiUnitStruct, GoodANDmuaStruct] = ImportKSdataC4(); % Add LaserStim to left side if wanted

   
    
    clustersAllST = uint32(readNPY('spike_clusters.npy'));           % For every spiketime, the cluster associated with it
    spike_amplitudes = uint32(readNPY('amplitudes.npy'));               % For every spiketime, the amplituded associated with it
    ts = uint32(readNPY('spike_times.npy'));           %convert all spike times from python to matlab, in samples (I think).
    ...
    [AllUnitStruct] = CreateUnivStruct(cluster, ts, clustersAllST, spike_amplitudes);
    ...
    [GoodUnitStruct] = CreateGoodStruct(GoodUnits, ts, clustersAllST, spike_amplitudes);
    ...
    [MultiUnitStruct] = CreateGoodStruct(MultiUnits, ts, clustersAllST, spike_amplitudes);
    for i = 1:length(MultiUnitStruct)
      ...
      [GoodANDmuaStruct] = CreateGoodStruct(GoodANDmua, ts, clustersAllST, spike_amplitudes);
