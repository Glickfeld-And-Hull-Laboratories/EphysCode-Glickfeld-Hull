function [BinaryStruct] = ReadInAllBinaryChunks(TimeLim)
...
SpikeTimeSec = double(ts)/SampRate(meta);              % create vector of all spike timestamps
    ...
    if TimeLim(2) == inf
    LastSamp = SampRate(meta)*SpikeTimeSec(end);
    else
        LastSamp = SampRate(meta)*TimeLim(2);
    end
    LastSamp = int64(LastSamp);
    %samp0 = SpikeTimeSec(1)*SampRate(meta);
    samp0 = TimeLim(1)*SampRate(meta);
    samp0 = int64(samp0);
    
    format long
    sizeGuess = LastSamp - samp0;
    
    MaxCounter = (LastSamp/SampRate(meta)-TimeLim(1))/ChunkSizeSeconds;
    
  
    ChunkSizeSamps = ChunkSizeSeconds * SampRate(meta);
    %StartTime = samp0*SampRate(meta);
    nsamp = int64(ChunkSizeSamps);
    %nsamp = samp0 + ChunkSizeSamps;
    BinaryStruct(1:length(chanList)) = struct ('chan', [], 'Binary', zeros(sizeGuess,1)-10);
    line45 = toc
    for n = 1:length(chanList)
        BinaryStruct(n).chan = chanList(n);
    end
    sampCounter = 1; %will start by indexing into first row
    counter = 1;
  
    while samp0 < LastSamp
        timecheck = [num2str(counter) ' of ' num2str(MaxCounter)]
        nsamp = samp0 + ChunkSizeSamps;
        if (nsamp + samp0) > (LastSamp) %adjust for last iteration when you are less than chunkTime from the end
            nsamp = LastSamp - samp0;    
        end
        line58= toc
   AllBinary = ReadBin(samp0, nsamp, meta, binName, path);
   fclose('all');
   ...
   %Deal With Gain
    ...
    fI2V = Int2Volts(meta);
     ...
     for i = 1:length(chanList) %chanlist aka [ch]
        ...
        line86 = ['loop ' num2str(i) ' time ' num2str(toc)]
        BinaryStruct(i).Binary(sampCounter:sampCounter+nsamp-1) = (AllBinary(i,:) * conv).'; %CHANGED from dataArray(j,:) to i.
        
     end
     clear AllBinary
    timecheck = [num2str(toc) ' sec']
     samp0 = samp0 + nsamp;
    counter = counter + 1;
    end
