...
TSwindow = int64(timestamps(( timestamps >= SampleTS(i)) & (timestamps <=(SampleTS(i) + dataLength))))
mininSamp = floor(.005*SampRate(meta));
mininSamp = int64(mininSamp)
miniWaveforms = zeros(mininSamp, length(TSwindow));
%reporter = SampleTS(i);
for i = 1:1 %length(TSwindow)
    %figure
samp0 = int64(SampleTS(i)*SampRate(meta));
...
TSwindowI = int64(TSwindow(i));
miniTS = ((TSwindowI - TSwindow(1))*int64(SampRate(meta))) : 1 : floor(.005*int64(SampRate(meta))-1); 
%reporter = TSwindow(i);
