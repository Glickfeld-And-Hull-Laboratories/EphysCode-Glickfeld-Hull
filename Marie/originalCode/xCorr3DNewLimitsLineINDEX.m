...
function ThreeDstruct = xCorr3DNewLimitsLineINDEX(struct, xmin, xmax, bwidth, unit1, unit2, limMin, limMax, color, SD, SDboo) %times in seconds, n unit of interest for struct (.unitID= string, .timestamps= vector 
...
FRbinIndex = find(([ThreeDstruct.LowFR] < round(preIntervalFR)), 1, 'first');
...
x = [ThreeDstruct(1).edges];
y = [ThreeDstruct.LowFR];
for n = 1:length(ThreeDstruct)
Array(n,:)= [ThreeDstruct(n).N];
end
imagesc(x, y, Array);
