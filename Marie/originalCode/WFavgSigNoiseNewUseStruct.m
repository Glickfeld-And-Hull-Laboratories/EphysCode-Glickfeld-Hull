...
function [SigNoise2, AvgWvF, StDev] = WFavgSigNoiseNewUseStruct(TimeGridA, TimeGridB, struct, BinaryStruct, dataLength, timeLim, unit)
...
if (~isnan(timeLim))
    timestamps = timestamps(timestamps < (timeLim(2) - .0021)); %time limit timestamps with enough room to collect the waveforms within the time limits
    timestamps = timestamps(timestamps > (timeLim(1)+.0011));
end
...
ch = chan +1 %switch from channel to index

   dataArray = [BinaryStruct(ch).Binary];
   NoiseMedian = median(dataArray)
   imhere = 'imhere';
   MAD = median(abs(dataArray - NoiseMedian));  
   sizeMAD = size(MAD);
   sizeForNoise = size(dataArray)
else 
   MAD = NaN;
end
...
%dataArray = [BinaryStruct.Binary];
% % % change y values to volts
% For an analog channel: gain correct saved channel ch (1-based for MATLAB).
ch = chan;

Waveforms(:,i) = dataArray(samp0:samp0 + nSamp-1);
end
