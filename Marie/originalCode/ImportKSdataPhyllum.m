...
%
%
function [AllUnitStruct, GoodUnitStruct, MultiUnitStruct, GoodANDmuaStruct, loc] = ImportKSdataPhyllum() % Add LaserStim to left side if wanted
%[cluster_struct, AllUnitStruct, GoodUnitStruct, MultiUnitStruct, GoodANDmuaStruct, GoodUnitStructSorted, GoodANDmuaStructSorted, AllUnitStructSorted] = ImportKSdataPhyllum() % Add LaserStim to left side if wanted
    ...
    C = textscan(fileID, '%s%s%s%s%s%s%s%s%s%s%s%s%s%s%s%s%s%s%s%s%s%s%s%s%s%s%s%s%s%s%s%s%s%s%s%s%s%s%s%s%s%s%s%s%s%s%s%s%s%s%s%s%s%s', 'HeaderLines', 1, 'Delimiter', {'\t'}, 'MultipleDelimsAsOne', 0); % removed blank space delimiter and adjusted # of colums to match phyllum
   C = readcell('cluster_info.tsv', 'FileType', 'text');
    ...
    layer = C(:,find(strcmp(Fields, 'final_neuron_layer')));
    snr = C(:,find(strcmp(Fields, 'SNR')));
    PC_dist = C(:,find(strcmp(Fields, 'PC_distance')));
    DCS_dist = C(:,find(strcmp(Fields, 'PC_DCS_distance')));
    MF_dist= C(:,find(strcmp(Fields, 'MF_distance')));
    if isempty(MF_dist)
        MF_dist= C(:,find(strcmp(Fields, 'MFB_distance')));
    end
    %distFromPC = {}
    %cluster = C{1};
    %group = C{6};
    ...
    cluster_struct = struct('cluster', cluster, 'group', group, 'layer', layer, 'chanel', channel, 'depth', depth, 'FR', FR, 'PC_dist', PC_dist, 'DCS_dist',  DCS_dist, 'MF_dist', MF_dist, 'snr', snr);
    % 
...
%             end
            ...
            uDCS_dist(j) = (cluster_struct(j).DCS_dist);
            uMF_dist(j) = (cluster_struct(j).MF_dist);
