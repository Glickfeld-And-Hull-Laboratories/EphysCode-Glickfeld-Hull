function BiggestWFstruct = multiWFPlotterOnly2(list, structstruct, figboo, fig2boo, fig3boo)
...
for m = 1:length(list)%length(structstruct)
    ...
    for n = 1:length(struct)
        %plot(time+ struct(n).X/4000, struct(n).AvgWf+struct(n).Y*Scale/30, color);
        ...
        text(textx, texty, [num2str(struct(n).Chan)]); 
         ...
         if fig3boo == 1
    struct= structstruct(k).MultiChanWFStruct_50msStimPre;
     color = 'b';
     for n = 1:length(struct)
        plot(time+ struct(n).X/4000, struct(n).AvgWf+struct(n).Y*Scale/30, color);
     ...
     struct= structstruct(k).MultiChanWFStruct_50msStimPost;
     color = 'c';
     for n = 1:length(struct)
        plot(time+ struct(n).X/4000, struct(n).AvgWf+struct(n).Y*Scale/30, color);
    ...
    color = 'm';
    for n = 1:length(struct)
        plot(time+ struct(n).X/4000, struct(n).AvgWf+struct(n).Y*Scale/30, 'Color', color);
    ...
    for n = 1:length(struct)
        plot(time+ struct(n).X/4000, struct(n).AvgWf+struct(n).Y*Scale/30, 'Color', color);
        scatter(time(60)+ struct(n).X/4000, struct(n).AvgWf(60)+struct(n).Y*Scale/30, 'k*');
