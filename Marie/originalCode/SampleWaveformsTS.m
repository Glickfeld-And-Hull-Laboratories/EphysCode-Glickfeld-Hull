...
%will take sample & average traces based on vector of timestamps, so you
%can use a trigger.
function [time, Waveforms, SampleTS] = SampleWaveformsTS( dataLength, n, Trig, prestim, chan)
title1 = inputname(3);
...
timestamps = Trig;  %% Make vector, TimeStamps, that has timestamps from unit.     
...
timestamps = (timestamps - prestim); % catch the begining of the waveform
...
figure %to pup up with new figure
hold on
for p = 1:n
