function [FR, WF, AvgWvF_postTG, time] = TestForMedina(unit, struct, trigger, TimeGridA, TimeGridB, xmin, xmax, TrigMin, TrigMax, TimeLim1, TimeLim2, ISIxaxis, binsize, color, nametag, JuiceTimes, LickTimes, RTs, JuiceBin, map)
...
DrugLines = 0;
   ...
   DrugTime3 = TimeLim2(2); %This really should be names DrugTimeSec3
    ...
    DrugName1 = 'Blockers';
    DrugName2 = 'Blocker Segment start';
    DrugName3 = 'End Blocker Segment';
    DrugName4 = 'acsf';
...
[time, WF_stim, ~] = SampleWaveformsTimeLimTGzerogo(TimeGridB, TimeGridB+.1, struct, .003, 100, TimeLim1, unit, channel, 0);
hold off
...
%[~, ~, Lim2Wfs, ~] = MultiChanWF(struct, .003, 100, TimeLim2, NaN, NaN, unit, map, 'g', 0, 1, Scale);
    end
%[~, ~, StimWFs, ~] = MultiChanWF( struct, .003, 100, TimeLim1, TimeGridB, TimeGridB+.1, unit, map, 'm', 0, 1, Scale);
%hold on
%for n = length(StimWFs)
%plot(time+StimWFs(n).X/5000, StimWFs(n).AvgWf + StimWFs(n).Y * StimWFs(n).Scale /30, 'm');
%end
hold off
...
%ISI panel
nexttile
ISIstructTimeGridTimeLim(TimeGridA, TimeGridB, struct, unit, ISIxaxis, TimeLim1, binsize, 'k', 1);
hold on
if doublechart == 1
...
ylabel('Hz');
xlabel('sec');
xline(0, 'b', 'LineWidth', 2, 'Labe', 'Laser');
if ~isnan(ylimitResp)
ylim(ylimitResp);
end
%yline(130, 'r');
...
if AmpHist == 1
    nexttile
    AmplitudeHistNew(unit, struct, [TimeLim1(1) timeEnd], color)
    title('Amplitude');
...
hold on
if doublechart == 1
    OneUnitHistStructTimeLim(trigger, unit, struct, TrigMin, TrigMax, binsize, TimeLim2, color2, .7);
end
title('Resp To Laser')
ylabel('Hz');
xlabel('msec');
...
if raster == 1
nexttile
RasterMatrix = OrganizeRasterSpikesNew(struct, trigger, unit, -(TrigMin), TrigMax);
title('Resp to Laser')
ylabel('Trial');
xlabel('msec');
