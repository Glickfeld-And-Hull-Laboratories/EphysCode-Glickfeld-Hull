function AddUnitToTrace (TimeWindow, struct, unit, chansFromChan, color);
% TimeWindow = [tmin, tmax]
% Unit = unit vector = list of timestamps for unit
unitIN = find([struct.unitID] == unit);
timestamps = extractUnitTimestamps(struct, unit);
timestamps = timestamps -.0005; % to catch beginning of unit
chan = struct(unitIN).channel + chansFromChan; %how many channels away from the official channel for this unit?
...
t = t.';
p = plot(t, miniWaveforms(:,k), 'color', color);
