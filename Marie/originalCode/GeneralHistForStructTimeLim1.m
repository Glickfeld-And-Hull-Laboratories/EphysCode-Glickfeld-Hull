...
function [meanLine, EXClatency, INHlatency, respType] = GeneralHistForStructTimeLim1(TS1, n, struct, xmin, xmax, timeLim, bwidth, color, StDevMult)  
...
N = FreqHist(N, edges, length(TS1), color, 1);
[meanLine, stdevLine] = StDevLine(N, edges);
UpLine = meanLine + StDevMult*stdevLine;
yline(UpLine, 'g', 'LineWidth', 2);
DownLine = meanLine - StDevMult*stdevLine;
if DownLine >0
yline(DownLine, 'y', 'LineWidth', 2);
end
reporter = N;
reporter2 = edges;
