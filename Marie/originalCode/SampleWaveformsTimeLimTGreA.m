...
% timeLim = [0, inf] where 0 is begining to use and inf is the max.
function [time, Waveforms, SampleTS] = SampleWaveformsTimeLimTGreA(TimeGridA, TimeGridB, struct, dataLength, n, timeLim, unit, chan)
title1 = ['Unit ' num2str(unit)];
title2 = ['chan ' num2str(chan)];
title3 = [num2str(timeLim(1))];
title4 = [ num2str(timeLim(2))];
title_ = [title1 ' on ' title2 ' between \n ' title3 ' and ' title4 ' TGrealigned'];
...
preTime = .001 % step back a little before each timestamp to catch the begining of the waveform
minimaWindowSize = .00025; % one tenth ms on either side of timestamp to find spike minima
peakWin = [((preTime - minimaWindowSize)*SampRate(meta)) ((preTime + minimaWindowSize)*SampRate(meta))]
timestamps = (timestamps - preTime); % catch the begining of the waveform
binTzero = preTime * SampRate(meta); %
...
samp0 = timestamps(randn)*SampRate(meta);
samp0 = int64(samp0);
...
WaveformsAdj = reAlignWF(Waveforms, SampleTS, time, preTime, int64(SampRate(meta)), peakWin, n);

%get average waveform
AvgWvF = avgeWaveforms(WaveformsAdj);

figure
hold on
for p = 1:n
    plot(time, WaveformsAdj(:,p), 'k');
    %ThisWave = WaveformsAdj(:,p);
     %TF = islocalmin(ThisWave);
    %plot(time(TF),ThisWave(TF),'r*');
end
plot(time, AvgWvF, 'r');
...
%WaveformsAdj = reAlignWF(Waveforms, SampleTS, time, preTime, int64(SampRate(meta)), peakWin);
