function AddUnitToTrace_fixedChan (TimeWindow, struct, unit, channelA, color);
...
timestamps = timestamps -.0002; % to catch beginning of unit
chan = channelA; %how many channels away from the official channel for this unit?
...
mininSamp = floor(.0006*SampRate(meta)); % .001 is size of unit trace
mininSamp = int64(mininSamp);
miniWaveforms = zeros(mininSamp, length(TSwindow));
miniTS = 1:1:floor(.0006*int64(SampRate(meta)));
miniTS = miniTS.';
x = miniTS;
...
t = t.';
plot(t, miniWaveforms(:,k), 'color', color);
%figure
