function [AlignedWaveforms, AlignedAverage, shiftTime] = AlignWaveformsXcorr2(Waveforms)
...
[centermin, Icentermin] = min(AVG);
 ...
 OriginalSize = max(avgeWaveforms(AlignedWaveforms))- min(avgeWaveforms(AlignedWaveforms))*1000;
...
[centermax, Icentermax] = max(AVG);
 ...
 NewSize = max(avgeWaveforms(AlignedWaveforms2))- min(avgeWaveforms(AlignedWaveforms2))*1000;
