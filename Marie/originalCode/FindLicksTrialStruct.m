...
function [JuiceLicks] = FindLicksNew(TrialStruct, level1, level2)
%clear
...
JuiceLicks = cell(length(TrialStruct)+1,5);
JuiceTimes = [TrialStruct.JuiceTime].';
ToneTimes = [TrialStruct.ToneTime].';
delay = mode(JuiceTimes - ToneTimes);
for n = 1:length(TrialStruct)
    if TrialStruct(n).TrialType == 't'
        TrialTimes(n) = ToneTimes(n)+delay;
        JuiceLicks{n,3} = 'ToneAlone';
    end
    if TrialStruct(n).TrialType == 'j'
        TrialTimes(n) = JuiceTimes(n);
        JuiceLicks{n,3} = 'JuiceAlone';
    end
    if  TrialStruct(n).TrialType == 'b'
        TrialTimes(n) = JuiceTimes(n);
        JuiceLicks{n,3} = 'ToneJuice';
    end
end
for m = 1:length(TrialTimes)+1 % change to length(TrialTimes) after testing
    
    %set samples to extract before and after juice delivery time
    if m == 1 %if it's the very first juicetime
        readMin = int64((TrialTimes(1)- 30-pre)*SampRate(meta));
        trialStartTime = TrialTimes(1)- 30-pre;
        if readMin < 0
           readMin = 0;
        end
        readMax = int64((TrialTimes(1)-pre)*SampRate(meta));
    elseif m > length(TrialTimes) %if it's the very last juice time
        readMin = int64((TrialTimes(m-1)-pre)*SampRate(meta));
        trialStartTime = TrialTimes(m-1) - pre;
        readMax = int64((TrialTimes(m-1)+30-pre)*SampRate(meta));
        if readMax > fTime*SampRate(meta)
           readMax = fTime*SampRate(meta);
        end
    else %juicetime is neither first nor last
        if TrialTimes(m)-TrialTimes(m-1) > 60 %juicetime has too long of juice pause after to use next JuiceTime to end licking data
        readMin = int64((TrialTimes(m-1)-pre)*SampRate(meta));
        trialStartTime = TrialTimes(m-1)-pre;
        readMax = int64((TrialTimes(m-1)+30-pre)*SampRate(meta));
        ...
        readMin = int64((TrialTimes(m-1)- pre)*SampRate(meta));
        trialStartTime = TrialTimes(m-1) - pre;
        readMax = int64((TrialTimes(m)-pre)*SampRate(meta));
        end
    end
