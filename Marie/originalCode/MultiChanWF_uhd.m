function [time, MainWaveforms, MultiChanWFStruct, Scale] = MultiChanWF_uhd(struct, dataLength, catchTime, n, timeLim, TimeGridA, TimeGridB, unit, map, color, PlotSamps, PlotMean, ScalePass)
...
if step == 0
   ...
   % f = warndlg('No spikes in WF mean calculation window. AvgWF = 0');
    ...
    counter = 0;
for j = 1:384   %for each chan we want to examine, previously j = (CenterChan - 75):(CenterChan + 75)
%if ~isempty(timestamps)
    if j < 0
        continue
    end
...
if PlotSamps == 1
    hold on
plot(time+X/1000, Waveforms+Y*Scale/5, 'Color', [.5 .5 .5]);
end
if PlotMean == 1
    hold on
plot(time+X/1000, AvgWvF+Y*Scale/5, color);
...
if PlotMean == 1
    ...
    X = map(jIndex).xcoord - map(MapIndexCenter).xcoord;
    Y = map(jIndex).ycoord - map(MapIndexCenter).ycoord;
plot(time+X/1000, BiggestWF+Y*Scale/5, color, 'LineWidth', 2);
end
...
textx = time(end/3) + X/1000;
texty = max(AvgWvF + Y*Scale/5)+5*(std(std(Waveforms)));
