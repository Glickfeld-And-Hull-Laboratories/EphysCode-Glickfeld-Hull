function SummaryStruct = SummaryStructMakerClassical(unitList, ID_label, IDtype, MLIexpertType, figName, Struct, PhyllumBoo, TimeLim,  MEH_chanMap, TrialStructAdj, PCpairList, AllLicksAdj)
%
ACGxlim = [-.3 .3];
ISIxlim = [0 .1];
WFlength = .003;
...
[FR, channel, TimeLim1, wfStructStruct, Nisi, edgesISI, Nacg, edgesACG] = ACGandWFTL(unitList(n), Struct, NaN, NaN, WFlength, catchTime, ACGxlim(1), ACGxlim(2), ISIxlim, [TimeLim], .001, 'k', figName, MEH_chanMap, 1);
SummaryStruct(counter).unitID = unitList(n);
...
SummaryStruct(counter).TrialStructAdj = TrialStructAdj;
SummaryStruct(counter).AllLicksAdj = AllLicksAdj;
index = find([Struct.unitID] == unitList(n));
SummaryStruct(counter).timestamps = Struct(index).timestamps;
if PhyllumBoo
    SummaryStruct(counter).PC_dist = Struct(index).PC_dist;
    SummaryStruct(counter).DCS_dist = Struct(index).DCS_dist;
    SummaryStruct(counter).MF_dist = Struct(index).MF_dist;
    SummaryStruct(counter).layer = Struct(index).layer;
    SummaryStruct(counter).snr = Struct(index).snr;
end
SummaryStruct(counter).IDtype = IDtype;
SummaryStruct(counter).MLIexpertID = MLIexpertType;
if ~isempty(PCpairList)
SummaryStruct(counter).PCpair = PCpairList(n);
else
    SummaryStruct(counter).PCpair =[];
end
counter = counter + 1
end

for n = 1:length(SummaryStruct)
    clear('Sizer') %added 2/28/23
...
if mini <lowlim || mini > highlim
    fprintf(['alignment error unit ' num2str(SummaryStruct(n).unitID) '\n'])
    ...
    BiggestWFstruct.unit = SummaryStruct(n).unitID;
