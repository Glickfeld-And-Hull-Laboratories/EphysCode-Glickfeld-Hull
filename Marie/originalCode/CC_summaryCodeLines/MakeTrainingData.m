...
RTs = [TrainingData(g).(F1{p}).TrialStruct.RTt];
bRTs = RTs(strcmp({TrainingData(g).(F1{p}).TrialStruct.TrialType}, 'b') | strcmp({TrainingData(g).(F1{p}).TrialStruct.TrialType}, 'j')); %only RTts on 'b' trials
RTs(isinf(RTs) | isnan(RTs)) = [];
...
histogram ([TrainingData(g).(F1{p}).LickOnsetOutside.TrialTime] , [-1:.05:45], 'FaceAlpha', .5, 'FaceColor', C(3,:))
end
if ~isempty([TrainingData(g).(F1{p}).LickOnsetBefore])
histogram ([TrainingData(g).(F1{p}).LickOnsetBefore.TrialTime] , [-1:.05:45], 'FaceAlpha', .5, 'FaceColor', C(3,:))
end
xline(0, 'g');
xline(.682, 'c');
xlim([-1 3])
