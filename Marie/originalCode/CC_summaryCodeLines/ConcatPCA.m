...
for n = 1:length(Summary)
    ...
    N_all(n,:) = (N_all(n,:) - mean(N_all(n,:)))/std(N_all(n,:));
end
...
clear N_SS
for n = 1:length(SS)
    [N_SS(n,:), edges] = OneUnitHistStructTimeLimLineINDEX(RecordingList(SS(n).RecorNum).JuiceTimes, n, SS, xmin, xmax, bwidth, [0 inf], 4, 'k', NaN, 0, 0);
    N_SS(n,:) = smoothdata([N_SS(n,:)], 'sgolay', smoothVal);
    N_SS(n,:) = (N_SS(n,:) - mean(N_SS(n,:)))/std(N_SS(n,:));
end
[U_ss, S_ss, V_ss] = svd(N_SS, 'econ');
for n = 1:3
...
for n = 1:length(SS([SS.RecorNum] == k))
    [N_SS(n,:), edges] = OneUnitHistStructTimeLimLineINDEX(RecordingList(k).JuiceTimes, n, SS([SS.RecorNum] == k), xmin, xmax, bwidth, [0 inf], 4, 'k', NaN, 0, 0);
    N_SS(n,:) = smoothdata([N_SS(n,:)], 'sgolay', smoothVal);
    N_SS(n,:) = (N_SS(n,:) - mean(N_SS(n,:)))/std(N_SS(n,:));
end
[U_ss, S_ss, V_ss] = svd(N_SS, 'econ');
...
legend({num2str(length(Summary)); num2str(length(SS)); num2str(length(SS([SS.RecorNum] == 1))); num2str(length(SS([SS.RecorNum] == 2))); num2str(length(SS([SS.RecorNum] == 3))); num2str(length(SS([SS.RecorNum] == 4))); num2str(length(SS([SS.RecorNum] == 5)))})
...
%  shadedErrorBar2(edges(1:end-1), mean(N_SS), std(N_SS)/sqrt(size(N_SS, 1)), 'LineProp', 'b');
%   %plot(edges(1:end-1), mean(N_CS), 'k');
%    shadedErrorBar2(edges(1:end-1), mean(N_CS), std(N_SS)/sqrt(size(N_CS, 1)), 'LineProp', 'k');
%    %plot(edges(1:end-1), mean(N_MLI), 'm');
%     shadedErrorBar2(edges(1:end-1), mean(N_MLI), std(N_SS)/sqrt(size(N_MLI, 1)), 'LineProp', 'm');
%     %plot(edges(1:end-1), mean(N_Gol), 'g');
%      shadedErrorBar2(edges(1:end-1), mean(N_Gol), std(N_SS)/sqrt(size(N_Gol, 1)), 'LineProp', 'g');
%      %plot(edges(1:end-1), mean(N_MF), 'r');
%       shadedErrorBar2(edges(1:end-1), mean(N_MF), std(N_SS)/sqrt(size(N_MF, 1)), 'LineProp', 'r');
