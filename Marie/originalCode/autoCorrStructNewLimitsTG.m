...
function [N, edges] = autoCorrStructNewLimitsTG(TimeGridA, TimeGridB, struct, xmin, xmax, bwidth, unit, TimeLim, color, FaceAlpha) %times in seconds, n unit of interest for struct (.unitID= string, .timestamps= vector 
...
[N, edges] = FreqHist(N, edges, length(Unit1), color, FaceAlpha, 1);
...
%tiledlayout(flow);
    ...
    f = warndlg('No spikes in ACG window. ');
end
