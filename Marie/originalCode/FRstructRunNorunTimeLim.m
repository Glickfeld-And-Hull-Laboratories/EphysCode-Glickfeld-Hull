function [FRate, FRrun, FRstop, TS2] = FRstructRunNorunTimeLim(ifrun, struct, burstLim, TimeLim, unit)
...
channel = [struct(n).channel];

TS2 = TS2((TimeLim(1) < TS2) & (TS2 < TimeLim(2)));



% remake time grid within time limits
if TimeLim(2)-TimeLim(1) > 0        % To catch when I blank out a unit by setting TimeLim1 = TimeLim2- this loop doesn't end until the end of the function.
ifruntimes = ifrun(:,1);
ifRunIndex = find((ifruntimes(:,1) < TimeLim(2)) & (ifruntimes > TimeLim(1)));
ifrun = ifrun(ifRunIndex(1):ifRunIndex(end),:);
...
title_ = strcat('Hz over time is', titleTr_, ' chan ', num2str(channel));
...
if TimeLim(1) == TimeLim(2)             % For units where it wasn't stable for long enough during the recording, set metrics = NaN and push an error message
    charUnit = num2str(unit);
    statement = ['The Time Limits are equal. ' charUnit ' was skipped.'];
    disp(statement)
    FRate = NaN;
    FRrun = NaN;
    FRstop = NaN;
