...
function  [N, edges, useDeltas] = OneUnitHistStructTimeLimLineINDEX(TS1, n, struct, xmin, xmax, bwidth, timeLim, STDEV, color, DisplayName, plotBoo, stdevlineboo)
...
%tiledlayout(flow);
    ...
    N = NaN(1,int64((range(end)-range(1))/bwidth));
    edges = NaN(1,int64((range(end)-range(1))/bwidth+1));
    L1 = NaN;
end
