...
%
%
function [cluster_struct, GoodUnits, MultiUnits, GoodANDmua, AllUnits, AllUnitStruct, GoodUnitStruct, MultiUnitStruct, GoodANDmuaStruct] = ImportKSdataNew(); % Add LaserStim to left side if wanted
    ...
    C = textscan(fileID, '%s %s %s %s %s %s %s %s %s %s %s %s %s %s %s %s %s %s %s %s %s %s %s %s %s %s %s %s %s', 'HeaderLines', 1, 'Delimiter', {'\t', '\b'}); % removed blank space delimiter and adjusted # of colums to match phyllum
    cluster = C{1};
    group = C{23};
    channel = C{15};
    depth = C{17};
    ...
    for i = 1:length(MultiUnitStruct)
      ...
      [GoodANDmuaStruct] = CreateGoodStruct(GoodANDmua, SpikeTimes, clustersAllST);
