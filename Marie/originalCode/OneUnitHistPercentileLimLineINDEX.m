...
function  [N, edges, L1] = OneUnitHistPercentileLimLineINDEX(TS1, n, struct, xmin, xmax, bwidth, timeLim, STDEV, color, DisplayName, plotBoo, stdevlineboo, SortItem, Pcntile, direct)
...
Ypctile = prctile(SortItem,Pcntile);
if strcmp(direct, 'above')
    TS1 = TS1(SortItem > Ypctile)
elseif strcmp(direct, 'below')
    TS1 = TS1(SortItem < Ypctile)
else
    error('Specify percentile with *above* or *below* \n')
end
TS1= TS1.';
...
[N, ~] = FreqLine(N, edges, length(TS1), color, DisplayName, plotBoo);
L1 = length(TS1);
% because we are counting through the trials with the spiketimes, not the triggers
%length(TS1)
