...
function [time, Waveforms] = GetWaveformsAtTimes(dataLength, chan, setTS)
...
timestamps = (timestamps); % don't catch the begining of the waveform
...
if ~isempty(timestamps)
    Waveforms = zeros(nSamp, length(setTS));
for i = 1:length(setTS)
samp0 =(setTS(i))*SampRate(meta);
samp0 = int64(samp0);
SampleTS(i) = setTS(i);
