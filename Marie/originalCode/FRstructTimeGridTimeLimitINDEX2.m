function [FRate, TimeGrid] = FRstructTimeGridTimeLimitINDEX2(TimeGridA,TimeGridB, TimeLim, struct, unit, color, plotboo, bwidth)
...
for f = 1:length(FR)
    ...
    if AllSpikes >0
    TimeTotal = TS2(end) - TimeLim(1)  %this assumes your end time lim might be higher than your recording. If not, adjust here.
    TimeTotal
    end
end
...
%figure;
%bar (FR, 1, color);
histogram (TS2, 'Binwidth', bwidth, 'Facecolor', color, 'Linestyle', 'none', 'FaceAlpha',1, 'Normalization', 'countdensity')
box off;
ax = gca; 
ax.TickDir = 'out';
