...
function reporter = xcorrTGjittercor(TimeGridA, TimeGridB, struct, jitterLim, xmin, xmax, bwidth, unit1, unit2, limMin, limMax, color) %times in seconds, n unit of interest for struct (.unitID= string, .timestamps= vector 
%*******************jitterLim must be evenly divisible by TimeGridB(1) - TimeGridA(1)***********************
...
for p = 1:length(TimeGridA)
    for n = 1:(TimeGridWindow/jitterLim) %-1)?
        nwindowspikes = Unit1(((TimeGridA + (n-1)*jitterLim) < Unit1) & (Unit1 < (TimeGridA + (n)*jitterLim) ));


%**************Do I need to space randomly placed spikes some distance
%apart? Also, how to determine acceptance bands......
