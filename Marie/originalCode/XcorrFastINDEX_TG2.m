...
function [N, edges] = XcorrFastINDEX_TG2(struct, xmin, xmax, bwidth, unit1, unit2, TimeGridA, TimeGridB, limMin, limMax, color, plotBoo, SD, SDboo) %times in seconds, n unit of interest for struct (.unitID= string, .timestamps= vector 
...
TimeGridB = TimeGridB((TimeGridB < limMax) & (TimeGridB > limMin));
TimeGridA = TimeGridB - TimeGridWindow;
if (TimeGridB(1)-TimeGridWindow)< limMin
    TimeGridA(1) = limMin;
end
