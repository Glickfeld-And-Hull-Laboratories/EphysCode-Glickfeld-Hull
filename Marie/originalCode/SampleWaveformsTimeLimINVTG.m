...
function [time, Waveforms, SampleTS] = SampleWaveformsTimeLimINVTG(TimeGridC, TimeGridD, struct, dataLength, n, timeLim, unit, chan)
title1 = ['Unit ' num2str(unit)];
title2 = ['chan ' num2str(chan)];
title3 = [num2str(timeLim(1))];
title4 = [ num2str(timeLim(2))];
title_ = [title1 ' on ' title2 ' between ' title3 ' and ' title4 'TG'];
...
TimeGridWindow = TimeGridD(1)-TimeGridC(1);         
TimeGridD = TimeGridD((TimeGridD < timeLim(2)) & (TimeGridD > timeLim(1)));
TimeGridC = TimeGridD - TimeGridWindow;
if (TimeGridD(1)-TimeGridWindow)< timeLim(1)
    TimeGridC(1) = timeLim(1);
end

timestamps = TimeGridUnitInverse(TimeGridC, TimeGridD, timestamps); %take only timestamps outside of 
