...
function [N, edges] = autoCorrStructNewLimits(struct, xmin, xmax, bwidth, unit, TimeLim, color) %times in seconds, n unit of interest for struct (.unitID= string, .timestamps= vector 
...
%N = N/2; % because you count everything n(spikes)*2, once ahead of the
%spike and once behind it. %Put this in freqHist
[N, edges] = FreqHist(N, edges, length(Unit1), color, 1, 1);
