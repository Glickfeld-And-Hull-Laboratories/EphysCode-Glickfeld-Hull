function [FR, WF, wfStructStruct] = CellCharWorkupDrugStruct2(unit, struct, trigger, TimeGridA, TimeGridB, xmin, xmax, TrigMin, TrigMax, TimeLim1, TimeLim2, DrugLinesStruct, ISIxaxis, binsize, color, nametag, JuiceTimes, LickTimes, RTs, JuiceBin, map)
...
%FR panel
nexttile 
fr= FRstruct2(unit, struct, 1, 'k', 1);
title('FR')
ylabel('Hz');
xlabel('sec');
...
%ISI panel
nexttile;
ISIstructTimeGridTimeLim(TimeGridA, TimeGridB, struct, unit, ISIxaxis, TimeLim1, binsize, color, 2, .5);
hold on
if doublechart == 1
...
hold on
if doublechart == 1
    OneUnitHistStructTimeLim(trigger, unit, struct, TrigMin, TrigMax, binsize, TimeLim2, 4, color2, .7, 1);
end
title('Resp To Laser');
ylabel('Hz');
xlabel('msec');
