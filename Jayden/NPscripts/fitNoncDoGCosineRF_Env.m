function [params, modelRF, fitInfo] = fitNoncDoGCosineRF_Env( ...
...
function r = weightedResidualDoGCos(p, XYdata, datav, wvec, gaussianMode)
    ...
    pen_ds = scaleReg * lambda_ds * (deltaSigma / (sc + eps));
...
% p = [Ac As sigmaC deltaSigma tau theta x0 y0 f phi dx dy]
    ...
    Xg = cos(theta + pi/2) * Xc + sin(theta + pi/2) * Yc;
