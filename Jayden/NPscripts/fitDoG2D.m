...
function [params, modelRF, fitInfo] = fitDoG2D(data, opts)
    ...
    % Data-driven initial amplitude
    % if abs(minResp) > abs(maxResp)
    %     Ac0 = minResp;
    % else
    %     Ac0 = maxResp;
    % end
    % 
    % p0 = [Ac0, 0.5*abs(Ac0), range(xCoords)/4, range(xCoords)/2, 0, 0];
    ...
    lb = [0, 0, eps, eps, min(xCoords), min(yCoords)]; %remove Ac bound, but enforce As is the opposite of Ac
    ub = [ Inf,  Inf, max(nx,ny), max(nx,ny), max(xCoords), max(yCoords)];
