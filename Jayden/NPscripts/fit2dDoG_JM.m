function results = fit2dDoG_JM(data,options)
...
switch options.parallel
    case true % here begins the parallised process
        ...
        for i_sp = 1:size(starts,1)
            para_res(i_sp) = parfeval(@customNonConcentricEllipticalDoGFit,1,patch,starts(i_sp,:),options.lb,options.ub,options.shape);
        end
...
for i_sp = 1:size(starts,1)
    ...
    switch options.parallel
        ...
        case false
            out = customNonConcentricEllipticalDoGFit(patch,starts(i_sp,:),options.lb,options.ub,options.shape);
            fetch_idx=i_sp;
    end
...
% --- DoG: skip Gabor-specific refinement ---
disp('Skipping getBetterSP (not defined for DoG model)');
        ...
        switch options.parallel
            case true
                ...
                for i_refit = 1:size(options.sigmas(options.sigmas~=0),2)
                    rf_res(i_refit) = parfeval(@customNonConcentricEllipticalDoGFit,1,patch,starts(end-size(options.sigmas(options.sigmas~=0),2)+i_refit,:),options.lb,options.ub,options.shape);
                end
                cancelFutures = onCleanup(@() cancel(para_res));
        end
        ...
        for i_rf = 1 : size(options.sigmas(options.sigmas~=0),2)
            switch options.parallel
                case true
                    ...
                    %end
                case false
                    out = customNonConcentricEllipticalDoGFit(patch,starts(end-size(options.sigmas(options.sigmas~=0),2)+i_rf,:),options.lb,options.ub,options.shape);
                    fetch_idx=i_rf;
            end
...
switch shape
    ...
    %% =========================================================
    % Nonconcentric Elliptical DoG (engine-compatible)
    %% =========================================================
    case 'elliptical'
        % Parameter order:
        % [Ac As sigmaC sigmaS tau theta x0 y0 dx dy]
    ...
    %% =========================================================
    % Circular / Equal (standard Gabor case)
    %% =========================================================
    case 'equal'
        % [a b x0 y0 sigma phi lambda phase]
...
function out = customNonConcentricEllipticalDoGFit(data, sp, lb, ub, ~)

g_level = sp(end) + 1;
sp = sp(1:end-1);

patch = data(g_level).data;
patchv = patch(:);

u = reshape(data(1).x, [], 1);
v = reshape(data(1).y, [], 1);

dogFt = fittype( ...
    @(Ac, As, sc, ss, tau, theta, x0, y0, dx, dy, u, v) ...
        Ac .* exp(-( ...
            ( cos(theta)*(u-x0) + sin(theta)*(v-y0) ).^2 + ...
            ( tau * ( -sin(theta)*(u-x0) + cos(theta)*(v-y0) ) ).^2 ...
        ) / (2*sc^2)) ...
      - As .* exp(-( ...
            ( cos(theta)*(u-(x0+dx)) + sin(theta)*(v-(y0+dy)) ).^2 + ...
            ( tau * ( -sin(theta)*(u-(x0+dx)) + cos(theta)*(v-(y0+dy)) ) ).^2 ...
        ) / (2*ss^2)), ...
    'independent', {'u','v'}, ...
    'coefficients', {'Ac','As','sc','ss','tau','theta','x0','y0','dx','dy'} );


[params, gof] = fit([u, v], patchv, dogFt, ...
    'StartPoint', sp, ...
    'Lower', lb, ...
    'Upper', ub, ...
    'MaxIter', 1e4, ...
...
params_adj = postprocessDoG(params);
model = makeNonConcentricEllipticalDoG(params_adj, u, v);


r2 = getrsquare(data(1).data, reshape(model, size(data(1).data)));

out = struct( ...
    'fit_raw', params, ...
    'fit_adjusted', params_adj, ...
    'fit_evaluated', reshape(model, size(data(1).data)), ...
    'patch', reshape(model, size(data(1).data)), ...
    'gof', gof, ...
    'smooth', g_level, ...
    'shape', 'elliptical', ...
    'r2', r2 );
...
function G = makeNonConcentricEllipticalDoG(p, x, y)
    ...
    Xcp =  cos(p.theta)*Xc + sin(p.theta)*Yc;
    Ycp = -sin(p.theta)*Xc + cos(p.theta)*Yc;

    Xsp =  cos(p.theta)*Xs + sin(p.theta)*Ys;
    Ysp = -sin(p.theta)*Xs + cos(p.theta)*Ys;

    Gc = exp(-(Xcp.^2 + (p.tau*Ycp).^2) / (2*p.sc^2));
    Gs = exp(-(Xsp.^2 + (p.tau*Ysp).^2) / (2*p.ss^2));
...
function p = postprocessDoG(params)
    names = coeffnames(params);
    vals  = coeffvalues(params);
    p = struct();
    for i = 1:numel(names)
        p.(names{i}) = vals(i);
    end
