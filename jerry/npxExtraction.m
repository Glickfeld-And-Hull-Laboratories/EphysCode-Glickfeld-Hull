...
% Get autocorrelogram and violations
    ...
    dt = cell(length(lenTimes), 1);
    for i = lenTimes
        diffs = spikeTimesForISI - spikeTimesForISI(i);
        diffs(i) = [];
        dt{i} = diffs(abs(diffs) <= maxLag);
    end
    dt = vertcat(dt{:});
...
% Plot autocorrelogram
    figure(1);
        movegui('center')
        subplot(3,1,2)
            bar(acBinCenters, acCounts, 'hist'); 
            hold on
...
% If there are enough spikes to divide into chunks
    if step > 0
        ...
        starts = (0:num_samples-1) * step + 1;  % Start of each chunk
        randIdx = starts + randi(step, 1, num_samples) - 1;  % Random index within each chunk
        TSindex = randIdx';
    else
 ...
 % Extract waveforms from neural data file
    ...
    for i = 1:length(TSindex)
        ...
        SampleTS(i)     = spikeStart(TSindex(i,1)); % Store the spike timestamp used for this waveform (n th spike start time in seconds)
...
% Get sample waveforms
    ...
    if ~isempty(spikeStart)
        ...
        for i = 1:length(TSindex)
            ...
            dataArray   = ReadBin_tut_TH(fid, samp0, nSamp, meta)
