...
function runCatGTret_TH(date,mouse)
...
% Check if exactly one folder is found
    ...
    elseif numel(runFolders) > 1
        ...
        for i = 1:numel(runFolders)
            disp([num2str(i) ': ' runFolders{i}]);
        end
        run2Analyze = input('Choose run to analyze: ');
    else
        run2Analyze = 1;
    end
    disp(['Analyzing run ' num2str(run2Analyze) ': ' runFolders{run2Analyze}]);
% Get full run folder and name
    runFolder = fullfile(fullDataPath, runFolders{run2Analyze});

% Get run name minus experimental run digits
    runName = runFolders{run2Analyze};
...
% Execute commands
    cd('C:\Users\th352\Desktop\CatGTWinApp4.3\CatGT-win');
    system(cmd1);
    system(cmd2);
