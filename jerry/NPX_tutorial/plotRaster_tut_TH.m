...
function plotRaster_tutorial(gratingRespMatrix, gratingRespOFFMatrix, unitIdx, dirIdx)

    % Check if indices are valid
    if unitIdx > size(gratingRespMatrix, 1) || dirIdx > size(gratingRespMatrix, 2)
        error('Invalid unit or direction index');
    end
    
    % Get spike times for the specified unit and direction
    baselineSpikeTimes  = gratingRespOFFMatrix{unitIdx, dirIdx}; % Spikes in baseline (-0.5 to 0s)
    trialsSpikeTimes    = gratingRespMatrix{unitIdx, dirIdx};    % Spikes in stimulus (0 to 1s)
    ...
    xlim([-0.2 2]); % Shows baseline (-0.5 to 0s) and stimulus (0 to 2s)
