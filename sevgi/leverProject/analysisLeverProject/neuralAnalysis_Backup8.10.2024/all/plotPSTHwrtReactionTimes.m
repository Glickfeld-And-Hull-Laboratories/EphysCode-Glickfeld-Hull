function meanSmthFast = plotPSTHwrtReactionTimes(cellTypeName, cellFastReactSpikes, cellSlowReactSpikes, fastMarkedEvent, slowMarkedEvent, sTitle, sFileName, totalFastTrialCount, totalSlowTrialCount, startTime, endTime)
        globalsAll;
        ...
        plot(edgesPlt, arrSmthSlow, 'LineWidth', 0.5, 'Color', [0.1, 0.2, 1 0.5]);
        plot(edgesPlt, arrSmthFast, 'LineWidth',0.5, 'Color', [1, 0.2, 0.1, 0.5]);
        
        % Plot mean firing rates
        meanSmthSlow = mean(arrSmthSlow,1);
        meanSmthFast = mean(arrSmthFast,1);
        pLegSlow = plot(edgesPlt, meanSmthSlow, 'LineWidth', 3, 'Color', [0.1, 0.2, 1, 0.8]);
        pLegFast = plot(edgesPlt, meanSmthFast, 'LineWidth',3, 'Color', [1, 0.2, 0.1, 0.8]);
        ...
        ylim([-0.1 max(max(meanSmthSlow),max(meanSmthFast))*1.2]);
        grid on
        set(gca,'TickDir','out');
