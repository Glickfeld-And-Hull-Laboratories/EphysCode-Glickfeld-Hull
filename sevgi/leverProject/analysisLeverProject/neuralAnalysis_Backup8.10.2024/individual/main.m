...
[unitSingle, unitMulti, unitNoise, unitUnprocessed] = readUnits();
...
if ~isempty(unitUnprocessedSorted)
    unprocessedIds = [unitUnprocessedSorted.id];
    unprocessedNeuronTypes = {unitUnprocessedSorted.neuronType};
else
    unprocessedIds = [];
    unprocessedNeuronTypes = {};
end

createFolders([unitSingleSorted.id],{unitSingleSorted.neuronType}, [unitMultiSorted.id],{unitMultiSorted.neuronType}, [unitNoiseSorted.id],{unitNoiseSorted.neuronType}, unprocessedIds, unprocessedNeuronTypes);
...
% This return section is commented out since it will overwrite these variables which are crutial to save unitGoodSorted in the next step (see SAVE ALL UNITS)
%             [spikeRatesHoldRandomAll{uid}, spikeRatesHoldFixedAll{uid}, spikeRatesHoldRandomHFM{uid}, spikeRatesHoldFixedHFM{uid}, spikeRatesReleaseRandomAll{uid}, spikeRatesReleaseFixedAll{uid}, ...
%                 spikeRatesReleaseRandomHFM{uid}, spikeRatesReleaseFixedHFM{uid}, spikeRatesTargetRandomAll{uid}, spikeRatesTargetFixedAll{uid}, spikeRatesTargetRandomHFM{uid}, spikeRatesTargetFixedHFM{uid}, ...
%                 responseTypeHoldFixedAll{uid}, responseTypeHoldFixedHFM{uid}, responseTypeReleaseFixedAll{uid}, responseTypeReleaseFixedHFM{uid}, responseTypeTargetFixedAll{uid}, responseTypeTargetFixedHFM{uid}] = ...
...
if ismember(ANALYSIS_STEP_2_0,ARR_DO_ANALYSES) 
    [unitSingleSorted, unitMultiSorted, dataSetNeedsToBeResaved] = acgAnalyses(unitSingleSorted, unitMultiSorted, unitNoiseSorted, unitUnprocessedSorted, preHoldTime, leverHoldTimes, leverReleaseTimesGLX, targetStimTimesGLX, arrStimTurnedOnTrials, allTrials, arrHitTrials, arrMissTrials, arrFaTrials);
end
...
if dataSetNeedsToBeResaved || (~loaded && ismember(ANALYSIS_STEP_1,ARR_DO_ANALYSES) && ismember(ANALYSIS_STEP_2_0,ARR_DO_ANALYSES) && UNIT_OF_INTEREST== -1)
    saveUnitswBehavVars(dataSetNeedsToBeResaved, unitSingleSorted, unitMultiSorted, unitNoiseSorted, unitUnprocessedSorted, leverHoldTimes, leverReleaseTimesGLX, targetStimTimesGLX, baselineStimTimesGLX, trialCutIndex, ...
