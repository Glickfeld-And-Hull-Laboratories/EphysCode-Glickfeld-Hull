function [suppressedPairs_CS_SS, pairs_CS_SS] = ccgAnalyses(unitGoodSorted, unitMuaSorted, preHoldTime, leverHoldTimes, leverReleaseTimesGLX, targetStimTimesGLX, arrStimTurnedOnTrials, allTrials, arrHitTrials, arrMissTrials, arrFaTrials)
    ...
    if UNIT_OF_INTEREST~= -1
        ...
        if ~isempty(unitOfInterest)
            ...
            if strcmp(unitOfInterest.expertLabel, NEURON_TYPE_CS) || strcmp(unitOfInterest.expertLabel, NEURON_TYPE_SS)
                pairs_CS_SS = findPairs(units, PAIR_TYPE_CS_SS, NEURON_TYPE_CS, NEURON_TYPE_SS, PAIR_CS_SS_MAX_LAYER_DISTANCE, unitOfInterest);
                if strcmp(unitOfInterest.expertLabel,NEURON_TYPE_CS)
                    otherNeuronType = NEURON_TYPE_SS;
                else
                    otherNeuronType = NEURON_TYPE_CS;
                end
                [suppressedPairs_CS_SS, ~] = plotCCGPairs(pairs_CS_SS, PAIR_TYPE_CS_SS, units, unitOfInterest.expertLabel, otherNeuronType, preHoldTime, leverHoldTimes, leverReleaseTimesGLX, targetStimTimesGLX, arrStimTurnedOnTrials, allTrials, arrHitTrials, arrMissTrials, arrFaTrials, 0);
            end
            if strcmp(unitOfInterest.expertLabel, NEURON_TYPE_DCS) || strcmp(unitOfInterest.expertLabel, NEURON_TYPE_SS)
                pairs_DCS_SS = findPairs(units, PAIR_TYPE_CS_SS, NEURON_TYPE_DCS, NEURON_TYPE_SS, PAIR_CS_SS_MAX_LAYER_DISTANCE, unitOfInterest);
                if strcmp(unitOfInterest.expertLabel,NEURON_TYPE_DCS)
                    otherNeuronType = NEURON_TYPE_SS;
                else
                    otherNeuronType = NEURON_TYPE_DCS;
                end
                [suppressedPairs_DCS_SS, ~] = plotCCGPairs(pairs_DCS_SS, PAIR_TYPE_CS_SS, units, unitOfInterest.expertLabel, otherNeuronType, preHoldTime, leverHoldTimes, leverReleaseTimesGLX, targetStimTimesGLX, arrStimTurnedOnTrials, allTrials, arrHitTrials, arrMissTrials, arrFaTrials, 0);
            end
            if strcmp(unitOfInterest.expertLabel, NEURON_TYPE_MF) || strcmp(unitOfInterest.expertLabel, NEURON_TYPE_GoC)
                pairs_MF_GO = findPairs(units, PAIR_TYPE_MF_GO, NEURON_TYPE_MF, NEURON_TYPE_GoC, PAIR_MF_GO_MAX_LAYER_DISTANCE, unitOfInterest);
                if strcmp(unitOfInterest.expertLabel,NEURON_TYPE_MF)
                    otherNeuronType = NEURON_TYPE_GoC;
                else
                    otherNeuronType = NEURON_TYPE_MF;
                end
                [suppressedPairs_MF_GO, ~] = plotCCGPairs(pairs_MF_GO, PAIR_TYPE_MF_GO, units, unitOfInterest.expertLabel, otherNeuronType, preHoldTime, leverHoldTimes, leverReleaseTimesGLX, targetStimTimesGLX, arrStimTurnedOnTrials, allTrials, arrHitTrials, arrMissTrials, arrFaTrials, 0);
            end
            if strcmp(unitOfInterest.expertLabel, NEURON_TYPE_GoC) || strcmp(unitOfInterest.expertLabel, NEURON_TYPE_SS)
                pairs_GO_SS = findPairs(units, PAIR_TYPE_GO_SS, NEURON_TYPE_GoC, NEURON_TYPE_SS, PAIR_GO_SS_MAX_LAYER_DISTANCE, unitOfInterest);
                if strcmp(unitOfInterest.expertLabel,NEURON_TYPE_GoC)
                    otherNeuronType = NEURON_TYPE_SS;
                else
                    otherNeuronType = NEURON_TYPE_GoC;
                end
                [suppressedPairs_GO_SS, ~] = plotCCGPairs(pairs_GO_SS, PAIR_TYPE_GO_SS, units, unitOfInterest.expertLabel, otherNeuronType, preHoldTime, leverHoldTimes, leverReleaseTimesGLX, targetStimTimesGLX, arrStimTurnedOnTrials, allTrials, arrHitTrials, arrMissTrials, arrFaTrials, 0);
            end
            if strcmp(unitOfInterest.expertLabel, NEURON_TYPE_BC_SC) || strcmp(unitOfInterest.expertLabel, NEURON_TYPE_SS)
                pairs_MLI_SS = findPairs(units, PAIR_TYPE_MLI_SS, NEURON_TYPE_BC_SC, NEURON_TYPE_SS, PAIR_MLI_SS_MAX_LAYER_DISTANCE, unitOfInterest);
                if strcmp(unitOfInterest.expertLabel,NEURON_TYPE_BC_SC)
                    otherNeuronType = NEURON_TYPE_SS;
                else
                    otherNeuronType = NEURON_TYPE_BC_SC;
                end
                [suppressedPairs_MLI_SS, ~] = plotCCGPairs(pairs_MLI_SS, PAIR_TYPE_MLI_SS, units, unitOfInterest.expertLabel, otherNeuronType, preHoldTime, leverHoldTimes, leverReleaseTimesGLX, targetStimTimesGLX, arrStimTurnedOnTrials, allTrials, arrHitTrials, arrMissTrials, arrFaTrials, 0);
            end
            if strcmp(unitOfInterest.expertLabel, NEURON_TYPE_MF) || strcmp(unitOfInterest.expertLabel, NEURON_TYPE_SS)
                pairs_MF_SS = findPairs(units, PAIR_TYPE_MF_SS, NEURON_TYPE_MF, NEURON_TYPE_SS, PAIR_MF_SS_MAX_LAYER_DISTANCE, unitOfInterest);
                if strcmp(unitOfInterest.expertLabel,NEURON_TYPE_MF)
                    otherNeuronType = NEURON_TYPE_SS;
                else
                    otherNeuronType = NEURON_TYPE_MF;
                end
                [suppressedPairs_MF_SS, ~] = plotCCGPairs(pairs_MF_SS, PAIR_TYPE_MF_SS, units, unitOfInterest.expertLabel, otherNeuronType, preHoldTime, leverHoldTimes, leverReleaseTimesGLX, targetStimTimesGLX, arrStimTurnedOnTrials, allTrials, arrHitTrials, arrMissTrials, arrFaTrials, 0);
            end            
            if strcmp(unitOfInterest.expertLabel, NEURON_TYPE_SS) || strcmp(unitOfInterest.expertLabel, NEURON_TYPE_DCN)
                ...
                if strcmp(unitOfInterest.expertLabel,NEURON_TYPE_SS)
                    otherNeuronType = NEURON_TYPE_DCN;
                else
                    otherNeuronType = NEURON_TYPE_SS;
                end
                [suppressedPairs_SS_DCN, ~] = plotCCGPairs(pairs_SS_DCN, PAIR_TYPE_SS_DCN, units, unitOfInterest.expertLabel, otherNeuronType, preHoldTime, leverHoldTimes, leverReleaseTimesGLX, targetStimTimesGLX, arrStimTurnedOnTrials, allTrials, arrHitTrials, arrMissTrials, arrFaTrials, 0);
            end

            % Now look at the cell types inhibiting each other or excited together (synchronous)
            if strcmp(unitOfInterest.expertLabel, NEURON_TYPE_BC_SC)
                pairs_MLI_MLI = findPairs(units, PAIR_TYPE_MLI_MLI, NEURON_TYPE_BC_SC, NEURON_TYPE_BC_SC, PAIR_MLI_MLI_MAX_LAYER_DISTANCE, unitOfInterest);
                [suppressedPairs_MLI_MLI, ~] = plotCCGPairs(pairs_MLI_MLI, PAIR_TYPE_MLI_MLI, units,NEURON_TYPE_BC_SC,NEURON_TYPE_BC_SC, preHoldTime, leverHoldTimes, leverReleaseTimesGLX, targetStimTimesGLX, arrStimTurnedOnTrials, allTrials, arrHitTrials, arrMissTrials, arrFaTrials, 0);
            end
            if strcmp(unitOfInterest.expertLabel, NEURON_TYPE_GoC)
