function checkNeuralChangeswrtLicks(unit, arrReactTimes, preHoldTime, fixedHoldStartsAtTrial, leverHoldTimes, leverReleaseTimesGLX, targetStimTimesGLX, baselineStimTimesGLX, ...
    lickOnsetTimesGLX, lickOffsetTimesGLX, rewardOnsetTimesGLX, rewardOffsetTimesGLX, arrStimTurnedOnTrials, allTrials, arrHitTrials, arrFaTrials, arrMissTrials)
        ...
        for indTrial=fixedHoldStartsAtTrial:length(allTrials)
            ...
            indActualTrial = find(arrHitTrials==indTrial);
        ...
        plot(edgesPlt, smtNoLickSpikeRates, 'LineWidth',1.4, 'Color', 'green');
        ...
        %scatter(edgesPlt,loSpikeRates,40, '.', 'green');
        plot(edgesPlt, smtLickSpikeRates, 'LineWidth',1.4, 'Color', 'red');
        ...
        title([unit.expertLabel ' id=' num2str(unit.id) ' layer=' unit.layer ' depth=' num2str(unit.depth) ' PSTH']);
        ...
        if ~all(smtNoLickSpikeRates==0) || ~all(smtLickSpikeRates==0)
            print([sLickTimeAnalysesPSTHFolder unit.expertLabel '_' num2str(unit.id) '_psth_LickvsNoLickPSTH.tif'], '-dtiff', '-r100');
            close all
        else
        ...
        plot(edgesPlt, smtSpikeRatesLickAligned, 'LineWidth',1.4, 'Color', 'red');
        ylabel('Spikes/s');
        ...
        set(gca,'TickDir','out');
        set(gca,'FontName','Times New Roman','FontWeight','bold', 'FontSize',PLOT_FONT_SIZE,'LineWidth',1.5)   
        title([unit.expertLabel ' id=' num2str(unit.id) ' layer=' unit.layer ' depth=' num2str(unit.depth) ' PSTH aligned to lick onset']);
        ...
        if ~all(smtNoLickSpikeRates==0) || ~all(smtLickSpikeRates==0)
            print([sLickTimeAnalysesPSTHFolder unit.expertLabel '_' num2str(unit.id) '_psth_LickAlignedPSTH.tif'], '-dtiff', '-r100');
            close all
        else
            logger.info('checkNeuralChangeswrtLicks', ['Either or both No Lick or Lick spikes for unit ' num2str(unit.id) ' were empty!']);
        end
