...
function [deviated, singleUnit] = correlogram(ccgType, unitRefID, unitTargetID, cellUnitRefSpikeTimesSec, cellUnitTargetSpikeTimesSec, trialCount, sTitle, isACG, expertLabelforACG)
    globals;
...
%         end
    ...
    if isACG && strcmp(expertLabelforACG,NEURON_TYPE_MF)
    ...
    singleUnit = 0;
    if isACG && ~strcmp(expertLabelforACG,NEURON_TYPE_MF) && refractoryViolationRate<=.05 % Refractory violation is 5 % for all cell types except MF
        singleUnit = 1;
    elseif isACG && strcmp(expertLabelforACG,NEURON_TYPE_MF) && refractoryViolationRate<=.1 % Refractory violation is 10 % for MFs
        singleUnit = 1;        
    end
    ...
    if (deviationValue > CCG_DEVIATION_CRITERION || isACG) && ~isempty(targetSpikeRates) && any(~isnan(targetSpikeRates))
        ...
        title([sHeader ' ContamRate=' num2str(refractoryViolationRate,'%.2f') ' (n=' num2str(trialCount) ') trials']);
...
%         title(sprintf(['Corrected (only within trial effects) ' sHeader ' ' sTitle ' Unit %d vs Unit %d'],unitRefID,unitTargetID)); 
        ...
        if isACG
            sPrintFolder = [pathToFigureFolder num2str(unitRefID)];
            if ~isempty(expertLabelforACG)
                sPrintFolder = [sPrintFolder '_' expertLabelforACG];
            end
