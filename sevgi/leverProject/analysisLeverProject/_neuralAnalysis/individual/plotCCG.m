...
function [suppressed, singleUnit] = plotCCG(ccgType, unitRefID, unitTargetID, unitRefSpikeTimesSec,unitTargetSpikeTimesSec, preHoldTime, leverHoldTimes, leverReleaseTimesGLX, visStimTimesGLX, arrStimTurnedOnTrials, arrSelectedTrials, whichCellTypes, isACG, expertLabelforACG)
    ...
    [suppressedClassic, singleUnit] = correlogram(ccgType, unitRefID, unitTargetID, unitRefSpikeTimesTrialITIAlignedToLeverHold, unitTargetSpikeTimesTrialITIAlignedToLeverHold, trialCount, ['Classic ' whichCellTypes], isACG, expertLabelforACG);
    
    %*************************** CORRELOGRAM FOCUSED ON LEVEL HOLD *******************************
    logger.info('plotCCG', ['trying ' ccgType ' HOLD']);
    [suppressedHold, ~] = correlogram(ccgType, unitRefID, unitTargetID, unitRefSpikeTimesAlignedToLeverHold, unitTargetSpikeTimesAlignedToLeverHold, trialCount, ['LeverHold ' whichCellTypes], isACG, expertLabelforACG);    
    %*************************** CORRELOGRAM FOCUSED ON LEVEL RELEASE *******************************
    logger.info('plotCCG', ['trying ' ccgType ' RELEASE']);
    [suppressedRelease, ~] = correlogram(ccgType, unitRefID, unitTargetID, unitRefSpikeTimesAlignedToLeverRelease, unitTargetSpikeTimesAlignedToLeverRelease, trialCount, ['LeverRelease ' whichCellTypes], isACG, expertLabelforACG);    
    %*************************** CORRELOGRAM FOCUSED ON VIS STIM *******************************
    logger.info('plotCCG', ['trying ' ccgType ' VIS STIM']);
    [suppressedVisStim, ~] = correlogram(ccgType, unitRefID, unitTargetID, unitRefSpikeTimesAlignedToVisStim, unitTargetSpikeTimesAlignedToVisStim, trialCount, ['VisStimChange ' whichCellTypes], isACG, expertLabelforACG);    
    ...
    %     end    
        [suppressedSalient, ~] = correlogram(ccgType, unitRefID, unitTargetID, unitRefSpikeTimesTrial, unitTargetSpikeTimesTrial, trialCount, ['SalientAroundLevRel' num2str(salientEventTime,3) ' ' whichCellTypes], isACG, expertLabelforACG);
    end
    suppressed = 0;
