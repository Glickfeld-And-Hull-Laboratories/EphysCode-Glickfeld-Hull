...
function pairs = findPairs(units, pairType, neuronType1, neuronType2, maxDistance, searchByUnit)
    globals;
    %NEURON_TYPE_MF NEURON_TYPE_BC_SC NEURON_TYPE_GoC NEURON_TYPE_UBC 
    %PAIR_TYPE_MF_SS PAIR_TYPE_MF_GO PAIR_TYPE_GO_SS
    
    pairs=[];
    
    if isempty(searchByUnit) % search all data set for all possible pairs
        for indX=1:length(units)
            if strcmp(units(indX).expertLabel,neuronType1) % found the first type
                for indY=1:length(units)
                    if strcmp(units(indY).expertLabel,neuronType2) && units(indY).id~=units(indX).id % found the second type
        ...
        if strcmp(searchByUnit.expertLabel,neuronType1)
        ...
        for ind=1:length(units)
            if strcmp(units(ind).expertLabel,searchType) && units(ind).id~=searchByUnit.id % found the second type
