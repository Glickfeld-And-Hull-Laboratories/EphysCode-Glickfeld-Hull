function [arrHitTrials, arrFaTrials, arrMissTrials, arrStimTurnedOnTrials, arrReqHoldTimes, arrHoldStarts, arrReactTimes, tooFastTime, reactTime, preHoldTime, fixedHoldStartsAtTrial, omissionTrials, nonOmissionTrials] = readBehavioralData(trialCutIndex)
    globals;
    ...
    if ~isempty(arrDays)
        for nDay=1:length(arrDays)
            ...
            holdStarts = cell2mat(input.holdStartsMs);
            ...
            arrHoldStarts = [arrHoldStarts holdStarts];
        ...
        if (SOFT_CUT~=Inf || HARD_CUT~=Inf) && ~isempty(trialCutIndex) % that means we soft-cut this recording and trial count hinted from SpikeGLX system            
            ...
            arrHoldStarts = arrHoldStarts(trialInds);
            ...
            if ~isempty(omissionTrials)
                omissionTrials = intersect(omissionTrials,trialInds)-trialInds(1)+1;
                nonOmissionTrials = intersect(nonOmissionTrials,trialInds)-trialInds(1)+1;
            end
