function saveUnitswBehavVars(unitGood, unitMua, unitNoise, unitUnprocessed, unitAll, leverHoldTimes, leverReleaseTimesGLX, targetStimTimesGLX, baselineStimTimesGLX, trialCutIndex, ...
...
%         if UNIT_OF_INTEREST== -1 % save all units             
            ...
            for cid=1:length(unitCategoryList)
                ...
                for uid=1:length(unitList)
                    ...
                    [spikeTimeAlignedToLeverHoldAll, spikeTimeAlignedToLeverReleaseAll, spikeTimeAlignedToTargetVisStimAll, spikeTimeAlignedToBaselineVisStimAll, ~, ~, ~, ~, ~, ~, ~, ~, ~, fixedHoldStartsAtRelativeTrialAll, ~] = ...
                    ...
                    if fixedHoldStartsAtRelativeTrialAll>0                        
                        spikeTimeHoldRandomAll = spikeTimeAlignedToLeverHoldAll(1:fixedHoldStartsAtRelativeTrialAll-1)';
                        spikeTimeHoldFixedAll = spikeTimeAlignedToLeverHoldAll(fixedHoldStartsAtRelativeTrialAll:end)';
                        spikeTimeReleaseRandomAll = spikeTimeAlignedToLeverReleaseAll(1:fixedHoldStartsAtRelativeTrialAll-1)';
                        spikeTimeReleaseFixedAll = spikeTimeAlignedToLeverReleaseAll(fixedHoldStartsAtRelativeTrialAll:end)';
                        
                        spikeTimeTargetRandomAll = spikeTimeAlignedToTargetVisStimAll(1:fixedHoldStartsAtRelativeTrialAll-1)';
                        spikeTimeTargetFixedAll = spikeTimeAlignedToTargetVisStimAll(fixedHoldStartsAtRelativeTrialAll:end)';
                    else
                    ...
                    spikeRatesHoldRandomAll = calculateFiringRates(spikeTimeHoldRandomAll, EDGES_HOLD, BIN_SIZE_PSTH);
                    [spikeRatesHoldFixedAll, responseTypeHoldFixedAll] = calculateFiringRates(spikeTimeHoldFixedAll, EDGES_HOLD, BIN_SIZE_PSTH);
                    spikeRatesReleaseRandomAll = calculateFiringRates(spikeTimeReleaseRandomAll, EDGES_RELEASE, BIN_SIZE_PSTH);
                    [spikeRatesReleaseFixedAll, responseTypeReleaseFixedAll] = calculateFiringRates(spikeTimeReleaseFixedAll, EDGES_RELEASE, BIN_SIZE_PSTH);
                    spikeRatesTargetRandomAll = calculateFiringRates(spikeTimeTargetRandomAll, EDGES_VIS_STIM, BIN_SIZE_PSTH);
                    [spikeRatesTargetFixedAll, responseTypeTargetFixedAll] = calculateFiringRates(spikeTimeTargetFixedAll, EDGES_VIS_STIM, BIN_SIZE_PSTH); 
                    ...
                    spikeRatesHoldRandomHit = calculateFiringRates(spikeTimeHoldRandomHit, EDGES_HOLD, BIN_SIZE_PSTH);
                    [spikeRatesHoldFixedHit, responseTypeHoldFixedHit] = calculateFiringRates(spikeTimeHoldFixedHit, EDGES_HOLD, BIN_SIZE_PSTH);
                    spikeRatesReleaseRandomHit = calculateFiringRates(spikeTimeReleaseRandomHit, EDGES_RELEASE, BIN_SIZE_PSTH);
                    [spikeRatesReleaseFixedHit, responseTypeReleaseFixedHit] = calculateFiringRates(spikeTimeReleaseFixedHit, EDGES_RELEASE, BIN_SIZE_PSTH);
                    spikeRatesTargetRandomHit = calculateFiringRates(spikeTimeTargetRandomHit, EDGES_VIS_STIM, BIN_SIZE_PSTH);
                    [spikeRatesTargetFixedHit, responseTypeTargetFixedHit] = calculateFiringRates(spikeTimeTargetFixedHit, EDGES_VIS_STIM, BIN_SIZE_PSTH); 
                    ...
                    spikeRatesHoldRandomFa = calculateFiringRates(spikeTimeHoldRandomFa, EDGES_HOLD, BIN_SIZE_PSTH);
                    [spikeRatesHoldFixedFa, responseTypeHoldFixedFa] = calculateFiringRates(spikeTimeHoldFixedFa, EDGES_HOLD, BIN_SIZE_PSTH);
                    spikeRatesReleaseRandomFa = calculateFiringRates(spikeTimeReleaseRandomFa, EDGES_RELEASE, BIN_SIZE_PSTH);
                    [spikeRatesReleaseFixedFa, responseTypeReleaseFixedFa] = calculateFiringRates(spikeTimeReleaseFixedFa, EDGES_RELEASE, BIN_SIZE_PSTH);
                    ...
                    spikeRatesHoldRandomMiss = calculateFiringRates(spikeTimeHoldRandomMiss, EDGES_HOLD, BIN_SIZE_PSTH);
                    [spikeRatesHoldFixedMiss, responseTypeHoldFixedMiss] = calculateFiringRates(spikeTimeHoldFixedMiss, EDGES_HOLD, BIN_SIZE_PSTH);
                    spikeRatesReleaseRandomMiss = calculateFiringRates(spikeTimeReleaseRandomMiss, EDGES_RELEASE, BIN_SIZE_PSTH);
                    [spikeRatesReleaseFixedMiss, responseTypeReleaseFixedMiss] = calculateFiringRates(spikeTimeReleaseFixedMiss, EDGES_RELEASE, BIN_SIZE_PSTH);
                    spikeRatesTargetRandomMiss = calculateFiringRates(spikeTimeTargetRandomMiss, EDGES_VIS_STIM, BIN_SIZE_PSTH);
                    [spikeRatesTargetFixedMiss, responseTypeTargetFixedMiss] = calculateFiringRates(spikeTimeTargetFixedMiss, EDGES_VIS_STIM, BIN_SIZE_PSTH);

                    %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
                    
                    newUnit.spikeRatesHoldRandomAll = spikeRatesHoldRandomAll;
                    newUnit.spikeRatesHoldFixedAll = spikeRatesHoldFixedAll;
                    newUnit.spikeRatesHoldRandomHFM = [spikeRatesHoldRandomHit; spikeRatesHoldRandomFa; spikeRatesHoldRandomMiss]; %spikeRatesHoldRandomHFM{uid};
                    newUnit.spikeRatesHoldFixedHFM = [spikeRatesHoldFixedHit; spikeRatesHoldFixedFa; spikeRatesHoldFixedMiss]; %spikeRatesHoldFixedHFM{uid};
                    newUnit.spikeRatesReleaseRandomAll = spikeRatesReleaseRandomAll;
                    newUnit.spikeRatesReleaseFixedAll = spikeRatesReleaseFixedAll;
                    newUnit.spikeRatesReleaseRandomHFM = [spikeRatesReleaseRandomHit; spikeRatesReleaseRandomFa; spikeRatesReleaseRandomMiss];
                    newUnit.spikeRatesReleaseFixedHFM = [spikeRatesReleaseFixedHit; spikeRatesReleaseFixedFa; spikeRatesReleaseFixedMiss];
                    newUnit.spikeRatesTargetRandomAll = spikeRatesTargetRandomAll;
                    newUnit.spikeRatesTargetFixedAll = spikeRatesTargetFixedAll;
                    newUnit.spikeRatesTargetRandomHFM = [spikeRatesTargetRandomHit; spikeRatesTargetRandomMiss];
                    newUnit.spikeRatesTargetFixedHFM = [spikeRatesTargetFixedHit; spikeRatesTargetFixedMiss];
    
                    newUnit.responseTypeHoldFixedAll = responseTypeHoldFixedAll;% responseTypeHoldFixedAll{uid};
                    newUnit.responseTypeHoldFixedHFM = [responseTypeHoldFixedHit, responseTypeHoldFixedFa, responseTypeHoldFixedMiss]; %responseTypeHoldFixedHFM{uid};
                    newUnit.responseTypeReleaseFixedAll = responseTypeReleaseFixedAll;
                    newUnit.responseTypeReleaseFixedHFM = [responseTypeReleaseFixedHit, responseTypeReleaseFixedFa, responseTypeReleaseFixedMiss];
                    newUnit.responseTypeTargetFixedAll = responseTypeTargetFixedAll;
                    newUnit.responseTypeTargetFixedHFM = [responseTypeTargetFixedHit, responseTypeTargetFixedMiss];
        
                    %%%%% CALCULATE REFRACTORINESS and DECIDE IF IT IS A SINGLE OR MULTI UNIT
                    [singleUnit, refractoryViolationRate, ~,~,~] = calculateRefractoryViolation(newUnit.spikeTimesSecs',allTrials, leverHoldTimes, leverReleaseTimesGLX, newUnit.neuronType, []);
                    
%                     if cid==1 && ~singleUnit % in the unitGood list
%                         logger.info('saveUnitswBehavVars', ['Consider removing unit=' num2str(newUnit.id) ' (' newUnit.neuronType ') into Multi units (Update in Phyllum and delete ' unitsAndVarsPath ' ) since singleUnit=' ...
%                             num2str(singleUnit) ' with refractoryViolationRate=%' num2str(refractoryViolationRate*100,'%.2f') ' refractoryViolationRateMF=%' num2str(refractoryViolationRateMF*100,'%.2f')]);                        
%                         %return;
%                     elseif cid==2 && singleUnit % in the unitMua list
%                         logger.info('saveUnitswBehavVars', ['Consider removing unit=' num2str(newUnit.id) ' (' newUnit.neuronType ') into Single units (Update in Phyllum and delete ' unitsAndVarsPath ' ) since singleUnit=' ...
%                             num2str(singleUnit) ' with refractoryViolationRate=%' num2str(refractoryViolationRate*100,'%.2f') ' refractoryViolationRateMF=%' num2str(refractoryViolationRateMF*100,'%.2f')]);
%                         if ~isempty(newUnit.neuronType) % since Unknown multi units can have low refractoriness but may or may not be MF, so prompt the log above and just countinue-do not quit
%                             %return;
%                         end
%                     end
% 
...
%                     elseif cid==2
%                         category='Multi Units ';
%                     end
%                     logger.info('saveUnitswBehavVars', [category num2str(uid) ' Id=' num2str(newUnit.id) ' singleUnit=' num2str(singleUnit) ...
%                         ' with refractoryViolationRate=%' num2str(refractoryViolationRate*100,'%.2f') ...
%                         ' refractoryViolationRateMF=%' num2str(refractoryViolationRateMF*100,'%.2f') ...
%                         ' type=' newUnit.neuronType]);
                    newUnit.singleUnit = singleUnit;
                    newUnit.refractoryViolationRate = refractoryViolationRate;
            ...
            save(unitsAndVarsPath,'unitGood', 'unitMua', 'unitNoise', 'unitUnprocessed', 'unitAll', 'leverHoldTimes', 'leverReleaseTimesGLX', 'targetStimTimesGLX', 'baselineStimTimesGLX', ...
