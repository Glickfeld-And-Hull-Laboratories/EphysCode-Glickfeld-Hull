...
%         end
    ...
    singleUnit = 0;
    if isACG && ~strcmp(neuronType,NEURON_TYPE_MFB) && (refractoryViolationRate<=REFRACTORY_VIOLATION_LIMIT || isnan(refractoryViolationRate)) % NaN means it has no violations. Refractory violation is 5 % for all cell types except MF
        singleUnit = 1;
    elseif isACG && strcmp(neuronType,NEURON_TYPE_MFB) && (refractoryViolationRate<=REFRACTORY_VIOLATION_LIMIT_MF || isnan(refractoryViolationRate)) % NaN means it has no violations. Refractory violation is 10 % for MFs
        singleUnit = 1;        
    end
