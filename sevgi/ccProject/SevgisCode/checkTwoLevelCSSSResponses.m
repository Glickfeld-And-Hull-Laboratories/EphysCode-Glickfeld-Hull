...
function [cellCSTimesResponsivePerDay, cellCSTimesNonResponsivePerDay, ...
    ...
    checkTwoLevelCSSSResponses(behavDataForRecordingDays, unitCSs, unitSSs, paramMouseId, paramDay)
    globals;
    ...
    %             [arrBehavTimesSelected, recordingDayCellAllLicksSelected] = findBehavioralTimes(paramMouseId, paramDay, behavDataForRecordingDays);
                if ~isempty(arrBehavTimesSelected) 
                    ...
                    % Find the CS responsive to first level of trial type
                    flagResponsive = isResponsive(unitCS.timestamps, behavDataForRecordingDay.TrialStruct);
                    if flagResponsive == 1

                        %%%%%% TWO LEVEL RESPONSIVENESS ANALYSES HERE %%%%%%%%%%%%%%%%%%
                        tempNaiveTrials = TRIALS_TO_INCLUDE_NAIVE;
                        tempNotNaiveTrials = TRIALS_TO_INCLUDE_NOTNAIVE;
                        TRIALS_TO_INCLUDE_NAIVE = TRIALS_TO_INCLUDE_NAIVE_LEVEL_TWO;
                        TRIALS_TO_INCLUDE_NOTNAIVE = TRIALS_TO_INCLUDE_NOTNAIVE_LEVEL_TWO;
                        [arrBehavTimesSelectedTwoLevel, recordingDayCellAllLicksSelectedTwoLevel, lickOnsetsSelectedTwoLevel] = findBehavioralTimes(paramMouseId, paramDay, behavDataForRecordingDay);
                        TRIALS_TO_INCLUDE_NAIVE = tempNaiveTrials;
                        TRIALS_TO_INCLUDE_NOTNAIVE = tempNotNaiveTrials;

                        if ~isempty(arrBehavTimesSelectedTwoLevel)
                            csTimesAlignedSelected = chunkAlignSpikeTimes(unitCS.timestamps, arrBehavTimesSelectedTwoLevel);
                            recordingDayCellAllLicksSelectedAligned = chunkAlignBehavTimes(recordingDayCellAllLicksSelectedTwoLevel, arrBehavTimesSelectedTwoLevel);                
                            lickOnsetsSelectedAligned = chunkAlignBehavTimes(lickOnsetsSelectedTwoLevel, arrBehavTimesSelectedTwoLevel);
                            ...
                            if flagResponsive == -1
                                ...
                                elseif FLAG_WAY_OF_COMPARISON_FOR_CS_RESPONSIVENESS == FLAG_WAY_OF_COMPARISON_FOR_CS_RESPONSIVENESS_DUAL
                                    ...
                                    logger.info('checkCSSSResponses', ['Less responsive unit=' num2str(unitCS.unitID) ' day ' num2str(day) ' mouse ' num2str(mouseId)]);
                                    cellCSTimesNonResponsive{length(cellCSTimesNonResponsive)+1} = csTimesAlignedSelected; 
                                    [unitSS, trialsCueResponsive] = findPair(unitSSs, recordingDayInd, unitCS, arrBehavTimesSelectedTwoLevel);
            
                                    if ~isempty(unitSS)
                                        ssTimesPairedWCueRespCS = chunkAlignSpikeTimes(unitSS.timestamps, arrBehavTimesSelectedTwoLevel(logical(trialsCueResponsive)));
                                        ssTimesPairedWCueNonRespCS = chunkAlignSpikeTimes(unitSS.timestamps, arrBehavTimesSelectedTwoLevel(logical(~trialsCueResponsive)));
