function [arrModulationMagnitudeRewRespCS, arrModulationMagnitudeCueRespCS, arrModulationMagnitudeRewResp] = ...
    ...
    arrModulations, arrModulationMagnitudeRew, arrModulationMagnitudeCue, ...
    isSinusoidal, localMaxs, localMins, ...
    ...
    if ~isempty(dayCSResponsiveAllMice)
        if FLAG_PLOT_CS
            ...
            plotComparisonPSTHs(dayCSResponsiveAllMice, arrModulations, arrModulationMagnitudeRew, arrModulationMagnitudeCue, ...
                isSinusoidal, localMaxs, localMins, ...
...
%                 if ~isempty(cellSSJuiceRespCueRespCSPerMouse) && ~all(indAnyEmpty)
            if ~isCellArraySomehowEmpty(daySSJuiceRespCueRespCSAllMice)
                [spikeRatesPre_JuiceRespCueResp, spikeRatesPost_JuiceRespCueResp] = ...
                    plotComparisonPSTHs(daySSJuiceRespCueRespCSAllMice, arrModulations, arrModulationMagnitudeRew, arrModulationMagnitudeCue, ...
                    isSinusoidal, localMaxs, localMins, ...
...
%                 if ~isempty(cellSSJuiceNonRespCueRespCSPerMouse) && ~all(indAnyEmpty)
            if ~isCellArraySomehowEmpty(daySSJuiceNonRespCueRespCSAllMice)
                ...
                plotComparisonPSTHs(daySSJuiceNonRespCueRespCSAllMice, arrModulations, arrModulationMagnitudeRew, arrModulationMagnitudeCue, ...
                isSinusoidal, localMaxs, localMins, ...
...
%                 if ~isempty(cellSSJuiceRespCueNonRespCSPerMouse) && ~all(indAnyEmpty)
            if ~isCellArraySomehowEmpty(daySSJuiceRespCueNonRespCSAllMice)
                ...
                plotComparisonPSTHs(daySSJuiceRespCueNonRespCSAllMice, arrModulations, arrModulationMagnitudeRew, arrModulationMagnitudeCue, ...
                isSinusoidal, localMaxs, localMins, ...
                individualLickRates, meanLickOnsets, semLickOnsets, meanCues, semCues, NEURON_TYPE, [], [], [], [], 1, []);
...
%                 if ~isempty(cellSSJuiceNonRespCueNonRespCSPerMouse) && ~all(indAnyEmpty)
            if ~isCellArraySomehowEmpty(daySSJuiceNonRespCueNonRespCSAllMice)
                ...
                plotComparisonPSTHs(daySSJuiceNonRespCueNonRespCSAllMice, arrModulations, arrModulationMagnitudeRew, arrModulationMagnitudeCue, ...
                isSinusoidal, localMaxs, localMins, ...
                individualLickRates, meanLickOnsets, semLickOnsets, meanCues, semCues, NEURON_TYPE, [], [], [], [], 1, []);
...
%             if ~isempty(cellSSJuiceRespCueRespCSPerMouse) && ~all(indAnyEmpty)
          if ~isCellArraySomehowEmpty(daySSJuiceRespCueRespCSAllMice)
            f = prePlot();  
            sTitle = [NEURON_TYPE ' trials ' sRangewOrwOutCS ' n = ' num2str(size(daySSJuiceRespCueRespCSAllMice,1)) ' ' sAddTitle];
            sFile = [pathToSSPsthFolder sLabel sDay '_' NEURON_TYPE '_wJResp_wCS' sRangewOrwOutCS '_alignedTo' sAlignedTo];
            
            [spikeRatesPre_JuiceRespCueResp, spikeRatesPost_JuiceRespCueResp] = ...
                plotComparisonPSTHs(daySSJuiceRespCueRespCSAllMice, arrModulations, 'Inc', 'Dec', ... %arrModulationMagnitudeRew, arrModulationMagnitudeCue, isSinusoidal, localMaxs, localMins, ...
                individualLickRates, meanLickOnsets, semLickOnsets, meanCues, semCues, NEURON_TYPE, sTitle, sFile, [], [], 1, dayDepths); % red
            postPlot(f, ['Time from ' sAlignedTo ' (s)'], sYLabel, -PRE_BEHAVIORAL_EVENT_PLOT, POST_BEHAVIORAL_EVENT_PLOT, MIN_SS, MAX_SS, sTitle, sFile);
...
%                 'Inc', 'Dec', individualLickRates, meanLickOnsets, semLickOnsets, meanCues, semCues, dayDepths, sYLabel, 1, []);
            ...
            if NORMALIZE_X_AXIS_FOR_EACH_LICK
                sAddSubTitle = ' SinvsRamp';
                f = prePlot();  

                sTitle = [NEURON_TYPE ' trials ' sRangewOrwOutCS ' n = ' num2str(size(daySSJuiceRespCueRespCSAllMice,1)) ' ' sAddTitle sAddSubTitle];
                sFile = [pathToSSPsthFolder sLabel sDay '_' NEURON_TYPE '_wJResp_wCS' sRangewOrwOutCS '_alignedTo' sAlignedTo ' ' sAddTitle sAddSubTitle];                
                [spikeRatesPre_JuiceRespCueResp, spikeRatesPost_JuiceRespCueResp] = ...
                    plotComparisonPSTHs(daySSJuiceRespCueRespCSAllMice, isSinusoidal, 'Sinusoidal', 'Others', ... %localMaxs, localMins, ...
                    individualLickRates, meanLickOnsets, semLickOnsets, meanCues, semCues, NEURON_TYPE, sTitle, sFile, [], [], 1, dayDepths); % red
                postPlot(f, ['Time from ' sAlignedTo ' (s)'], sYLabel, -PRE_BEHAVIORAL_EVENT_PLOT, POST_BEHAVIORAL_EVENT_PLOT, MIN_SS, MAX_SS, sTitle, sFile);
                ...
                sAddSubTitle = ' 1stvs2ndHalfMaxTuning';
                f = prePlot();  
                sTitle = [NEURON_TYPE ' trials ' sRangewOrwOutCS ' n = ' num2str(size(daySSJuiceRespCueRespCSAllMice,1)) ' ' sAddTitle sAddSubTitle];
                sFile = [pathToSSPsthFolder sLabel sDay '_' NEURON_TYPE '_wJResp_wCS' sRangewOrwOutCS '_alignedTo' sAlignedTo ' ' sAddTitle sAddSubTitle];                
                [spikeRatesPre_JuiceRespCueResp, spikeRatesPost_JuiceRespCueResp] = ...
                    plotComparisonPSTHs(daySSJuiceRespCueRespCSAllMice, arrModulationsMax, '2ndHalfMax', '1stHalfMax', ... %localMaxs, localMins, ...
                    individualLickRates, meanLickOnsets, semLickOnsets, meanCues, semCues, NEURON_TYPE, sTitle, sFile, [], [], 1, dayDepths); % red
                postPlot(f, ['Time from ' sAlignedTo ' (s)'], sYLabel, -PRE_BEHAVIORAL_EVENT_PLOT, POST_BEHAVIORAL_EVENT_PLOT, MIN_SS, MAX_SS, sTitle, sFile);
                ...
                sAddSubTitle = ' 1stvs2ndHalfMinTuning';
                f = prePlot();  
                sTitle = [NEURON_TYPE ' trials ' sRangewOrwOutCS ' n = ' num2str(size(daySSJuiceRespCueRespCSAllMice,1)) ' ' sAddTitle sAddSubTitle];
                sFile = [pathToSSPsthFolder sLabel sDay '_' NEURON_TYPE '_wJResp_wCS' sRangewOrwOutCS '_alignedTo' sAlignedTo ' ' sAddTitle sAddSubTitle];                
                [spikeRatesPre_JuiceRespCueResp, spikeRatesPost_JuiceRespCueResp] = ...
                    plotComparisonPSTHs(daySSJuiceRespCueRespCSAllMice, arrModulationsMin, '2ndHalfMin', '1stHalfMin', ... %localMins, localMins, ...
                    individualLickRates, meanLickOnsets, semLickOnsets, meanCues, semCues, NEURON_TYPE, sTitle, sFile, [], [], 1, dayDepths); % red
                postPlot(f, ['Time from ' sAlignedTo ' (s)'], sYLabel, -PRE_BEHAVIORAL_EVENT_PLOT, POST_BEHAVIORAL_EVENT_PLOT, MIN_SS, MAX_SS, sTitle, sFile);
...
%             if ~isempty(cellSSJuiceNonRespCueRespCSPerMouse) && ~all(indAnyEmpty)
          if ~isCellArraySomehowEmpty(daySSJuiceNonRespCueRespCSAllMice)
            ...
            sFile = [pathToSSPsthFolder sLabel sDay '_' NEURON_TYPE '_wNonJResp_wCS' sRangewOrwOutCS '_alignedTo' sAlignedTo];
            postPlot(f, ['Time from ' sAlignedTo ' (s)'], sYLabel, -PRE_BEHAVIORAL_EVENT_PLOT, POST_BEHAVIORAL_EVENT_PLOT, MIN_SS, MAX_SS, sTitle, sFile);
          end           
...
%             if ~isempty(cellSSJuiceRespCueNonRespCSPerMouse) && ~all(indAnyEmpty)
          if ~isCellArraySomehowEmpty(daySSJuiceRespCueNonRespCSAllMice)
            ...
            sFile = [pathToSSPsthFolder sLabel sDay '_' NEURON_TYPE '_wJResp_wNOCS' sRangewOrwOutCS '_alignedTo' sAlignedTo];            
            postPlot(f, ['Time from ' sAlignedTo ' (s)'], sYLabel, -PRE_BEHAVIORAL_EVENT_PLOT, POST_BEHAVIORAL_EVENT_PLOT, MIN_SS, MAX_SS, sTitle, sFile); 
          end
%             end
...
%             if ~isempty(cellSSJuiceNonRespCueNonRespCSPerMouse) && ~all(indAnyEmpty)
          if ~isCellArraySomehowEmpty(daySSJuiceNonRespCueNonRespCSAllMice)
            ...
            sFile = [pathToSSPsthFolder sLabel sDay '_' NEURON_TYPE '_wNonJResp_wNOCS' sRangewOrwOutCS '_alignedTo' sAlignedTo];            
            postPlot(f, ['Time from ' sAlignedTo ' (s)'], sYLabel, -PRE_BEHAVIORAL_EVENT_PLOT, POST_BEHAVIORAL_EVENT_PLOT, MIN_SS, MAX_SS, sTitle, sFile); 
          end            
...
%             %%%%% Plot Spectrum  %%%%%%%
    ...
    if ~isempty(dayCSNonResponsiveAllMice) 
        if FLAG_PLOT_CS
            ...
            plotComparisonPSTHs(dayCSNonResponsiveAllMice, arrModulations, arrModulationMagnitudeRew, arrModulationMagnitudeCue, ...
                isSinusoidal, localMaxs, localMins, ...
                individualLickRates, meanLickOnsets, semLickOnsets, meanCues, semCues, NEURON_TYPE_CS, sTitle, sFile, ['Time from ' sAlignedTo ' (s)'], [], 0, []);
