...
function [cellIndsPerDay, baselineFRPerDay, modulationFRsPerDay, responseMagnitudesPerDay] = ...
    checkUpDown(behavDataForRecordingDays, unitCSs, unitSSs, paramMouseId, paramDay, sLabel)
    globals;
...
%         if ~isempty(unitSlave)       
        if ~isempty(arrBehavTimesSelected) 
            ...
            baselineFRs = [baselineFRs baselineFR];
            modulationFRs = [modulationFRs modulationFR];
...
%         end
        ...
        if dayNext ~= day ||  (indLoop+1)==length(inds)
            if (indLoop+1)<=length(inds)
            ...
            if ~isempty(responseMagnitudes)
                ...
                logger.info('checkUpDown', ['day ' num2str(day) ' mouse ' num2str(mouseId) ' mean response=' num2str(mean(responseMagnitudes))]);
...
%                             
                    baselineFRPerDay = [baselineFRPerDay baselineFRs];
                    modulationFRsPerDay = [modulationFRsPerDay modulationFRs];
                    responseMagnitudesPerDay = [responseMagnitudesPerDay responseMagnitudes];
                    cellIndsPerDay = [cellIndsPerDay csInds];
%                 end
            else
                a=0;
            end
