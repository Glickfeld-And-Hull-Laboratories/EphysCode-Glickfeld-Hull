function plotMultipleSpikeWaveForm(unit, allWaveForms, samplingRate, sLocalTitle)
    ...
    channelsNonEmpty=chMatrix(chMatrix~=UNDEFINED);
    [~,indsShared] = intersect(CHANNEL_ORGANIZATION,channelsNonEmpty);
...
%     baselineWF = allWaveForms{1};
%     avgWaveForms1=cellfun(@mean,baselineWF(indsShared),'UniformOutput',false); % Baseline
%     avgWaveForms11=mean(cell2mat(avgWaveForms1),1); %reshape(avgWaveForms1,[1,PLOT_MATRIX_ROWS*PLOT_MATRIX_COLUMNS])'),1);
% 
%     drug1WF = allWaveForms{2};
%     avgWaveForms2=cellfun(@mean,drug1WF(indsShared),'UniformOutput',false); % First Drug
%     avgWaveForms22=mean(cell2mat(avgWaveForms2),1); %reshape(avgWaveForms2,[1,PLOT_MATRIX_ROWS*PLOT_MATRIX_COLUMNS])'),1);
% 
%     if ~isempty(MOMENT_OF_2ND_DRUG_PUT_IN)
%         avgWaveForms3=cellfun(@mean,allWaveForms{1,3},'UniformOutput',false); % Second Drug
%         avgWaveForms33=mean(cell2mat(reshape(avgWaveForms3,[1,PLOT_MATRIX_ROWS*PLOT_MATRIX_COLUMNS])'),1);
%     end
% 
%     avgWaveForms=[];
...
%     end
%     if ~isempty(MOMENT_OF_2ND_DRUG_PUT_IN) && ~isnan(avgWaveForms33)
...
%     absMax = max(abs(maxWaveForms),abs(minWaveForms));
    ...
    if ~all(all(allEmptyArr))
        legendOn = 0;
        mainChMax = [];
        mygca = gobjects(PLOT_MATRIX_ROWS*PLOT_MATRIX_COLUMNS,1);
        ax = gobjects(PLOT_MATRIX_ROWS*PLOT_MATRIX_COLUMNS,1);
        for row = 1:PLOT_MATRIX_ROWS
            for col = 1:PLOT_MATRIX_COLUMNS
                indRowCol = (row-1)*PLOT_MATRIX_COLUMNS + col;
                ax(indRowCol) = subplot(PLOT_MATRIX_ROWS, PLOT_MATRIX_COLUMNS, indRowCol);
...
%                         wfToPlot = waveForms(indsShared);
%                         wfToPlot = reshape(wfToPlot,PLOT_MATRIX_COLUMNS,[])';
                        if chMatrix(row,col)~=UNDEFINED
                            [indX_InCHORG, indY_InCHORG] = find(CHANNEL_ORGANIZATION==chMatrix(row,col));
                            waveForm = waveForms{indX_InCHORG, indY_InCHORG};
        ...
        %                         patch([xPlot fliplr(xPlot)], [waveFormMeanPlt fliplr(waveFormMaxPlt)], colors{eachWF}, 'FaceAlpha',0.1); % fill between                                                
                                ...
                                title(num2str(chMatrix(row,col)));
                                if chMatrix(row,col)==unit.ch % cos ch numbers start from zero in Phyllum                                                        
                                    yl = yticklabels;
        ...
        %                         set(gca,'TickDir','out');
                ...
                if ~all(cellfun(@isempty,legends)) && ~legendOn && row==PLOT_MATRIX_ROWS && col==PLOT_MATRIX_COLUMNS
    ...
    %                 ylim([uVCoeff*minWaveForms uVCoeff*maxWaveForms]);
        ...
        for indYL=1:size(yl,1)
            if abs(yl(indYL,1))>abs(yl(indYL,2)) && yl(indYL,1) < 0
                yl(indYL,:) = [-max(mainChMax) otherSide];
            elseif abs(yl(indYL,1))<abs(yl(indYL,2)) && yl(indYL,2) > 0
                yl(indYL,:) = [-otherSide abs(max(mainChMax))];    
            end
