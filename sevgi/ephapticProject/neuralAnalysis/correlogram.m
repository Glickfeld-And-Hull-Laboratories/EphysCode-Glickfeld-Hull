...
function [singleUnit, suppressed, synchEd] = correlogram(pairs, pairType, unitsMaster, unitsSlave, startTimeSecs, endTimeSecs, movingTimesToBeExcluded, sFileName, colorLine, superImpose)
    globals;
    ...
    if ~isempty(pairs)        
        for iPair=1:nPairs
            ...
            if ~isempty(unitMaster) && ~isempty(unitSlave)
                ...
                if ~isempty(movingTimesToBeExcluded)
                    movingTimesToBeExcludedCell = num2cell(movingTimesToBeExcluded,2);
                ...
                if ~isempty(unitMasterSpikeTimesSecWhole) && ~isempty(unitSlaveSpikeTimesSecWhole)
                    ...
                    if ~isempty(unitMasterSpikeTimesSec) && ~isempty(unitSlaveSpikeTimesSec)
                        ...
                        if length(unitMasterSpikeTimesMSec)>MAX_ARRAY_LENGTH && ~strcmp(pairType,ACG) % Check if max array size is not exceeded!
                            unitMasterSpikeTimesMSec = unitMasterSpikeTimesMSec(randperm(length(unitMasterSpikeTimesMSec), MAX_ARRAY_LENGTH));
                        elseif strcmp(pairType,ACG)
                            unitMasterSpikeTimesMSec = unitSlaveSpikeTimesMSec;
                        end
            ...
            %                 relativeSpkTimesMs = relativeSpkTimesMs(~isnan(relativeSpkTimesMs));
                        ...
                        if strcmp(pairType,ACG) && ~strcmp(unitMaster.neuronType,NEURON_TYPE_MFB) && (refractoryViolationRate<=REFRACTORY_VIOLATION_LIMIT || isnan(refractoryViolationRate)) % NaN means it has no violations. Refractory violation is 6 % for all cell types except MF
                            singleUnit = 1;
                        elseif strcmp(pairType,ACG) && strcmp(unitMaster.neuronType,NEURON_TYPE_MFB) && (refractoryViolationRateMF<=REFRACTORY_VIOLATION_LIMIT_MF || isnan(refractoryViolationRateMF)) % NaN means it has no violations. Refractory violation is 10 % for MFs
                            singleUnit = 1;
                        elseif strcmp(pairType,ACG) && isempty(unitMaster.neuronType) && (refractoryViolationRate<=REFRACTORY_VIOLATION_LIMIT || isnan(refractoryViolationRate)) % Unknown units can still be MF
                            singleUnit = 1;
                        end
                    
                        %%%%%%% TO FIND IF MLI_SS SUPPRESSION AMOUNT IS BIGGER THAN 4 STD %%%%%%%%%
                        indsSupp = find(edges>=MLI_SS_SUPPRESSION_RANGE(1)&edges<=MLI_SS_SUPPRESSION_RANGE(2));
                        ...
                        %TODO: Update this suppression criteria until you implement quiscent period and rate corrected CCG
                        ...
                        suppressed = any(abs(zScoredSuppSlaveSpikeRates)>=std4); % absolute suppression amount should be bigger than 4*STD
                        %%%%%%% TO FIND IF MLI_SS SUPPRESSION AMOUNT IS BIGGER THAN 4 STD %%%%%%%%%

                        %%%%%%% TO FIND IF MLI_MLI SYNCHRONIZATION AMOUNT IS BIGGER THAN 4 STD %%%%%%%%%
                        indsSynch = find(edges>=0,1); % get only zero(0) moment index
                        ...
                        zScoredSynchSlaveSpikeRates = (mean(synchSlaveSpikeRates)-mean(synchSlaveSpikeRatesBaseline))/std(synchSlaveSpikeRatesBaseline);
                        std4 = 4*std(synchSlaveSpikeRatesBaseline);
                        synchEd = any(zScoredSynchSlaveSpikeRates>=std4);
                        %%%%%%% TO FIND IF MLI_MLI SYNCHRONIZATION AMOUNT IS BIGGER THAN 4 STD %%%%%%%%%
...
%                         deviationValue = 10; %abs(sum(corrCounts(find(edges>=-CCG_DEVIATION_RANGE&edges<=CCG_DEVIATION_RANGE)))); % Find the suppression around zero
                        ...
                        if superImpose~=-1 % Sometimes I just don't need to plot, just get the suppression or synch values
                            if ~isempty(slaveSpikeRates) && any(~isnan(slaveSpikeRates)) % (deviationValue > CCG_DEVIATION_CRITERION || pairType == PAIR_TYPE_ACG) &&
                                sHeader = [sPreHeader ' CCG for ' num2str(unitSlaveID) ' (' unitSlave.neuronType ' depth=' num2str(unitSlave.depth) ' um) wrt ' num2str(unitMasterID) ' (' unitMaster.neuronType ' depth=' num2str(unitMaster.depth) ' um)'];
                                if strcmp(pairType,ACG)
                                    sSingle = 'Multi';
                                    if singleUnit
                                        sSingle = 'Single';
    ...
    %                             hstRaw = histogram(relativeSpkTimesMs,edges, 'FaceColor', colorLine, 'FaceAlpha', .6);
    ...
    %                             end
                                ...
                                minLim = mean(slaveSpikeRates)-6*std(slaveSpikeRates);
                                ...
                                title([sHeader ' FR=' num2str(mean(slaveSpikeRates),'%.2f') ' spk/s']);
