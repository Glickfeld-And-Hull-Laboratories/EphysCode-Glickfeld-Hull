function [needToSave, unitGoodSorted] = ccgAnalysesForStationaryVSRunning(unitGoodSorted, unitMuaSorted, unitNoiseSorted, movingTimes, sStationaryOrMoving)
    ...
    if ~isempty(unitOfInterestSlave) && ~isempty(unitOfInterestMaster)
            ...
            if isempty(MOMENT_OF_2ND_DRUG_PUT_IN)
                [~, ~, ~, ~, ~, ~, ~, ~, meanRateDrug] = correlogramRateCorrected([unitOfInterestMaster.id unitOfInterestSlave.id], SS_MLI, unitsAll, MOMENT_OF_1ST_DRUG_WASH_IN, Inf, movingTimes, FIRST_DRUG, 'r', 1);
...
%                 [~, ~, ~, ~, ~, ~, ~, ~, meanRateDrug1] = correlogramRateCorrected([unitOfInterestMaster.id unitOfInterestSlave.id], SS_MLI, unitsAll, MOMENT_OF_1ST_DRUG_WASH_IN, MOMENT_OF_2ND_DRUG_PUT_IN, movingTimes, FIRST_DRUG, 'b', 1);
                [~, ~, ~, ~, ~, ~, ~, ~, meanRateDrug] = correlogramRateCorrected([unitOfInterestMaster.id unitOfInterestSlave.id], SS_MLI, unitsAll, MOMENT_OF_2ND_DRUG_WASH_IN, Inf, movingTimes, SECOND_DRUG, 'r', 1);
                legend(BASELINE,'', FIRST_DRUG, '', SECOND_DRUG, '', "Color",'none');
            end

            FLAG_STATIONARY_VS_RUNNING = 1; % 0-Stationary 1-Running
            [~, ~, ~, ~, ~, ~, ~, ~, meanRateBaseline] = correlogramRateCorrected([unitOfInterestMaster.id unitOfInterestSlave.id], SS_MLI, unitsAll, 0, MOMENT_OF_1ST_DRUG_PUT_IN, movingTimes, BASELINE, 'b', 1);
            
            if isempty(MOMENT_OF_2ND_DRUG_PUT_IN)
                [~, ~, ~, ~, ~, ~, ~, ~, meanRateDrug] = correlogramRateCorrected([unitOfInterestMaster.id unitOfInterestSlave.id], SS_MLI, unitsAll, MOMENT_OF_1ST_DRUG_WASH_IN, Inf, movingTimes, FIRST_DRUG, 'm', 1);
                legend(BASELINE, '', FIRST_DRUG, '', "Color",'none');
            else
%                 [~, ~, ~, ~, ~, ~, ~, ~, meanRateDrug1] = correlogramRateCorrected([unitOfInterestMaster.id unitOfInterestSlave.id], SS_MLI, unitsAll, MOMENT_OF_1ST_DRUG_WASH_IN, MOMENT_OF_2ND_DRUG_PUT_IN, movingTimes, FIRST_DRUG, 'b', 1);
                [~, ~, ~, ~, ~, ~, ~, ~, meanRateDrug] = correlogramRateCorrected([unitOfInterestMaster.id unitOfInterestSlave.id], SS_MLI, unitsAll, MOMENT_OF_2ND_DRUG_WASH_IN, Inf, movingTimes, SECOND_DRUG, 'm', 1);
                legend(BASELINE,'', FIRST_DRUG, '', SECOND_DRUG, '', "Color",'none');
            end
            ...
            sFRs = ['FR_{Bsl}=' num2str(meanRateBaseline,'%.0f') ' FR_{Drug}=' num2str(meanRateDrug,'%.0f') ' spk/s'];            
            set(gca,'FontName','Times New Roman','FontWeight','bold', 'FontSize',PLOT_FONT_SIZE);
            title({sHeader, sFRs});
            h = findobj(gca,'Type','line');
            legend(h,'AllBlockersRun', 'BaselineRun', 'AllBlockersStat', 'BaselineStat');
    ...
    elseif ~isempty(UNIT_OF_INTEREST_SLAVES) && ~isempty(UNIT_OF_INTEREST_MASTERS)
            ...
            for ind=1:length(UNIT_OF_INTEREST_SLAVES)   
                ...
                if ~isempty(unitOfInterestSlave) && ~isempty(unitOfInterestMaster)
                    ...
                    if isempty(MOMENT_OF_2ND_DRUG_PUT_IN)
                        [~, ~, ~, ~, ~, ~, ~, ~, meanRateDrug] = correlogramRateCorrected([unitOfInterestMaster.id unitOfInterestSlave.id], ccgType, unitsAll, MOMENT_OF_1ST_DRUG_WASH_IN, Inf, movingTimes, FIRST_DRUG, 'r', 1);
                    else
...
%                         [~, ~, ~, ~, ~, ~, ~, ~, meanRateDrug1] = correlogramRateCorrected([unitOfInterestMaster.id unitOfInterestSlave.id], ccgType, unitsAll, MOMENT_OF_1ST_DRUG_WASH_IN, MOMENT_OF_2ND_DRUG_PUT_IN, movingTimesToBeExcluded, FIRST_DRUG, 'b', 1);
                        [~, ~, ~, ~, ~, ~, ~, ~, meanRateDrug] = correlogramRateCorrected([unitOfInterestMaster.id unitOfInterestSlave.id], ccgType, unitsAll, MOMENT_OF_2ND_DRUG_WASH_IN, Inf, movingTimes, SECOND_DRUG, 'r', 1);
                    end

                    FLAG_STATIONARY_VS_RUNNING = 1; % 0-Stationary 1-Running
                    [~, ~, ~, ~, ~, ~, ~, ~, meanRateBaseline] = correlogramRateCorrected([unitOfInterestMaster.id unitOfInterestSlave.id], ccgType, unitsAll, 0, MOMENT_OF_1ST_DRUG_PUT_IN, movingTimes, BASELINE, 'b', 1);
                    
                    if isempty(MOMENT_OF_2ND_DRUG_PUT_IN)
                        [~, ~, ~, ~, ~, ~, ~, ~, meanRateDrug] = correlogramRateCorrected([unitOfInterestMaster.id unitOfInterestSlave.id], ccgType, unitsAll, MOMENT_OF_1ST_DRUG_WASH_IN, Inf, movingTimes, FIRST_DRUG, 'm', 1);
                    else
                        % Commented out since excitatory blockers are usually same as baseline
%                         [~, ~, ~, ~, ~, ~, ~, ~, meanRateDrug1] = correlogramRateCorrected([unitOfInterestMaster.id unitOfInterestSlave.id], ccgType, unitsAll, MOMENT_OF_1ST_DRUG_WASH_IN, MOMENT_OF_2ND_DRUG_PUT_IN, movingTimesToBeExcluded, FIRST_DRUG, 'b', 1);
                        [~, ~, ~, ~, ~, ~, ~, ~, meanRateDrug] = correlogramRateCorrected([unitOfInterestMaster.id unitOfInterestSlave.id], ccgType, unitsAll, MOMENT_OF_2ND_DRUG_WASH_IN, Inf, movingTimes, SECOND_DRUG, 'm', 1);
                    end
                                
                    sHeader = ['CCG ' sStationaryOrMoving ' ' num2str(unitOfInterestSlave.id) ' (' unitOfInterestSlave.neuronType ' depth=' num2str(unitOfInterestSlave.depth) ' um) wrt ' num2str(unitOfInterestMaster.id) ' (' unitOfInterestMaster.neuronType ' depth=' num2str(unitOfInterestMaster.depth) ' um)'];
                    sFRs = ['FRBsln=' num2str(meanRateBaseline,'%.0f') ' FRDrug=' num2str(meanRateDrug,'%.0f') ' spk/s'];
                    set(gca,'FontName','Times New Roman','FontWeight','bold', 'FontSize',PLOT_FONT_SIZE);
                    title([sHeader sFRs]);
                    h = findobj(gca,'Type','line');
                    legend(h,'AllBlockersRun', 'BaselineRun', 'AllBlockersStat', 'BaselineStat');
        ...
        if ~isempty(pairs_MLI_SS)        
            for iPair=1:size(pairs_MLI_SS,1)
                ...
                if ~isempty(unitPairMLI) && ~isempty(unitPairSS)
                    ...
                    if isempty(MOMENT_OF_2ND_DRUG_PUT_IN)
                        [~, ~, ~, synchExcFirstDrug, synchInhFirstDrug, unitGoodSorted, readForTheFirstTime, ~, meanRateDrug] = correlogramRateCorrected([unitPairMLI.id unitPairSS.id], SS_MLI, unitGoodSorted, MOMENT_OF_1ST_DRUG_WASH_IN, Inf, movingTimes, FIRST_DRUG, 'r', 1);
                        if readForTheFirstTime
                            needToSave = 1;
                        end
                    else                        
...
%                         end
                        ...
                        [~, ~, ~, synchExcSecondDrug, synchInhSecondDrug, unitGoodSorted, readForTheFirstTime, ~, meanRateDrug] = correlogramRateCorrected([unitPairMLI.id unitPairSS.id], SS_MLI, unitGoodSorted, MOMENT_OF_2ND_DRUG_WASH_IN, Inf, movingTimes, SECOND_DRUG, 'r', 1);
                        if readForTheFirstTime
                            needToSave = 1;
                        end                        
                    end

                    FLAG_STATIONARY_VS_RUNNING = 1;
                    [~, ~, ~, synchExc, synchInh, unitGoodSorted, readForTheFirstTime, ~, meanRateBaseline] = correlogramRateCorrected([unitPairMLI.id unitPairSS.id], SS_MLI, unitGoodSorted, 0, MOMENT_OF_1ST_DRUG_PUT_IN, movingTimes, BASELINE, 'b', 1);                    
                    if readForTheFirstTime
                        needToSave = 1;
                    end

                    if isempty(MOMENT_OF_2ND_DRUG_PUT_IN)
                        [~, ~, ~, synchExcFirstDrug, synchInhFirstDrug, unitGoodSorted, readForTheFirstTime, ~, meanRateDrug] = correlogramRateCorrected([unitPairMLI.id unitPairSS.id], SS_MLI, unitGoodSorted, MOMENT_OF_1ST_DRUG_WASH_IN, Inf, movingTimes, FIRST_DRUG, 'm', 1);
                        if readForTheFirstTime
                            needToSave = 1;
                        end
                    else                        
...
%                         end
                        ...
                        [~, ~, ~, synchExcSecondDrug, synchInhSecondDrug, unitGoodSorted, readForTheFirstTime, ~, meanRateDrug] = correlogramRateCorrected([unitPairMLI.id unitPairSS.id], SS_MLI, unitGoodSorted, MOMENT_OF_2ND_DRUG_WASH_IN, Inf, movingTimes, SECOND_DRUG, 'm', 1);
                        if readForTheFirstTime
                            needToSave = 1;
                        end                        
                    end
