...
function pairs = findPairs(units, pairType, neuronType1, neuronType2, maxDistance, searchByUnit)
    ...
    if isempty(searchByUnit) % search all data set for all possible pairs
        ...
        elseif pairType == PAIRED_SS_MLI
            whichToAdd = find(~all(ismember(PAIRED_SS_MLI,pairs),2));
            pairs = [pairs; PAIRED_SS_MLI(whichToAdd,:)]; % also add manually found ones from globals    
        ...
        for ind=1:length(units)
            if strcmp(units(ind).neuronType,neuronType2) && units(ind).id~=searchByUnit.id % found the second type
        ...
        elseif pairType == PAIR_TYPE_GO_SS
            ...
            pairs = [pairs; PAIRED_GO_SS(whichToAdd,:)]; % also add manually found ones from globals
        elseif pairType == PAIRED_SS_MLI
            whichToAdd = find(any(ismember(PAIRED_SS_MLI,searchByUnit.id),2) & ~all(ismember(PAIRED_SS_MLI,pairs),2));
            pairs = [pairs; PAIRED_SS_MLI(whichToAdd,:)]; % also add manually found ones from globals        
        elseif pairType == PAIR_TYPE_SS_DCN
