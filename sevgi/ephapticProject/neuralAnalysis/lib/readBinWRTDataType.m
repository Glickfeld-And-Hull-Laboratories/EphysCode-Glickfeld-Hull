...
%
function dataArray = readBinWRTDataType(samples, nSamp, chOfInterestInPhyllum, fileSizeBytes, binName, path, sizeOfData, sConvertDataType)
    globals;

    chOfInterest = chOfInterestInPhyllum+1; % convert Matlab indices cos Phyllum indices starts from zero
    ...
    for i=1:length(samples)                
        ...
        if samples(i)<nFileSamp
            ...
            logger.info('readBinWRTDataType', ['samples(i)=' num2str(samples(i)) ' bigger than nFileSamp=' num2str(nFileSamp) ' at i=' num2str(i) ' th trial']);
        end
    end
    fclose(fid);
end % ReadBin