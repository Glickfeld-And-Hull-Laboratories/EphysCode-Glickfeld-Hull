function [singleUnit, refractoryViolationRate, refractoryViolationRateMF, relativeSpkTimesMsec, targetSpikeRates] = calculateRefractoryViolation(spikeTimesSecs, neuronType, edges) % allTrials, leverHoldTimes, leverReleaseTimesGLX, 
    globals;
...
%     end
% 
%     unitMasterSpikeTimesSecWhole = spikeTimesSecs;
%     unitSlaveSpikeTimesSecWhole = spikeTimesSecs; 

    meanSpikeRate = length(spikeTimesSecs)/(spikeTimesSecs(end)-spikeTimesSecs(1));
        
    unitSlaveSpikeTimesMSec = spikeTimesSecs*1000; % convert to ms
    unitMasterSpikeTimesMSec = spikeTimesSecs*1000; % convert to ms
    ...
    relativeSpkTimesMsec = unitSlaveSpikeTimesMSec' - unitMasterSpikeTimesMSec;
    relativeSpkTimesMsec(eye(size(relativeSpkTimesMsec))==1) = NaN; % set diagonal to NaN, cos its the difference with the spike itself, ACG is not interested in self-difference of the very same spike! It emerges as if there is a contamination at t=0!
    
    % constrain it within the ROI
    % These two lines were crashing Matlab
%                 relativeSpkTimesMsec(relativeSpkTimesMsec<=edges(1) & relativeSpkTimesMsec>=edges(end))=NaN; 
%                 relativeSpkTimesMsec = relativeSpkTimesMsec(~isnan(relativeSpkTimesMsec));
    % So swapped with this line
    relativeSpkTimesMsec = relativeSpkTimesMsec(relativeSpkTimesMsec>=edges(1) & relativeSpkTimesMsec<=edges(end));
    ...
    elseif strcmp(neuronType,NEURON_TYPE_MFB) && (refractoryViolationRateMF<=REFRACTORY_VIOLATION_LIMIT_MF || isnan(refractoryViolationRateMF)) % NaN means it has no violations. Refractory violation is 10 % for MFs
        singleUnit = 1;
    elseif isempty(neuronType) && (refractoryViolationRate<=REFRACTORY_VIOLATION_LIMIT || isnan(refractoryViolationRate)) % Never mind - have an err on the cautious side! % Unknown units can still be MF
        singleUnit = 1;
    end
