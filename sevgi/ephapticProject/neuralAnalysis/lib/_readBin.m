...
%
function dataArray = readBin(samples, nSamp, chOfInterest, fileSizeBytes, binName, path)
    globals;
    sizeOfInt16 = 2; % Filtered bin file is in the int16 data type format
    %nChan = str2double(meta.nSavedChans);

    nFileSamp = str2double(fileSizeBytes) / (sizeOfInt16 * NUM_OF_CHANNELS);
    ...
    for i=1:length(samples)                
        ...
        fseek(fid, samples(i) * sizeOfInt16 * NUM_OF_CHANNELS, 'bof');
