...
% Updated by SO Hull lab 9/7/2024: to read multi samples at once to speed up the raw data reading
...
%
% chOfInterest is different than readBin() function, which adds 1 to channel index to convert it into Matlab indexing convention
function dataArray = readNIBin(samples, nSamp, chOfInterest, meta, binName, path)
    ...
    for i=1:length(samples)                
        ...
        sizeA = [nChan, nSamp];
        fseek(fid, samples(i) * 2 * nChan, 'bof');
        tempData = fread(fid, sizeA, 'int16=>double');        
        dataArray(i,1:size(tempData,2)) = tempData(chOfInterest,1:size(tempData,2));        
    end
    fclose(fid);
end % ReadBin