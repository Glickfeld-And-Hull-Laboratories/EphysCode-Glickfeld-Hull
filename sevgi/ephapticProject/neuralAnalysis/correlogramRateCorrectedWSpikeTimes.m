%%%% PLOT CORRELOGRAM WITH GIVEN SPIKE TIMES(AUTO/CROSS) %%%%%%%%%%%%
...
function [singleUnit, suppressed, activated, synchExc, synchInh, units, readForTheFirstTime, refractoryViolationRateLlobet, meanSlaveSpikeRate] = ...
    correlogramRateCorrectedWSpikeTimes(pairs, pairType, unitMasterSpikeTimes, unitSlaveSpikeTimes, units, startTimeSecs, endTimeSecs, sWhichPhase, colorLine, superImpose, doRateCorrected)
    globals;

    if nargin==10 % means doRateCorrected is not passed, so get the default value
        doRateCorrected = 1;
    end
        ...
        for iPair=1:nPairs
            ...
            if ~isempty(unitMaster) && ~isempty(unitSlave)
                    ...
                    if ~isempty(startTimeSecs) && ~isempty(endTimeSecs) %nargin>6 &&
                        unitMasterSpikeTimesSec = unitMasterSpikeTimes(startTimeSecs<unitMasterSpikeTimes & unitMasterSpikeTimes<endTimeSecs)';
                        unitSlaveSpikeTimesSec = unitSlaveSpikeTimes(startTimeSecs<unitSlaveSpikeTimes & unitSlaveSpikeTimes<endTimeSecs)'; 
                        sPreHeader = sWhichPhase;
                    else                    
                        unitMasterSpikeTimesSec = unitMasterSpikeTimes';
                        unitSlaveSpikeTimesSec = unitSlaveSpikeTimes';
                    end
                    ...
                    if ~isempty(unitMasterSpikeTimesSec) && ~isempty(unitSlaveSpikeTimesSec)
                        ...
                        if isNotStoredPreviously(sWhichPhase, units, masterId, slaveId, unitMasterID, unitSlaveID, doRateCorrected) 
                            ...
                            units = storeRates(sWhichPhase, units, masterId, slaveId, unitMasterID, unitSlaveID, slaveSpikeRates, doRateCorrected);
                            readForTheFirstTime = 1;
                            %%%%%%%%%%%%%%%%%%% Calculate Regular or Rate Corrected CCG %%%%%%%%%%%%%%%%%%%%%%
                        else
                            slaveSpikeRates = getRates(sWhichPhase, units, slaveId, unitMasterID, doRateCorrected);
                        end
                        ...
                        %DONE:Quiscent period implemented %TODO: Update this suppression criteria until you implement quiscent period and rate corrected CCG
