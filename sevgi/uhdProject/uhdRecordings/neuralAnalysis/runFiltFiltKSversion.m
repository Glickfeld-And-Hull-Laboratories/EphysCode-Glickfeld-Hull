...
%         bufferedRawDataGCorr = [buffer;rawDataGCorr;buffer];
        ...
        %%%%%%% TODO: These are NOT TESTED YET!!! KILOSORT VERSION OF FILTERING %%%%%%%%%%%%%%%%%%%%%%%
        % This part added so that it is the same way Kilosort filters the raw data
        rawDataGCorr = rawDataGCorr';
        meanSubtractedRawData = rawDataGCorr - mean(rawDataGCorr,1); % filtered sensor signal        
        % CAR, common average referencing by median
        if CAR
            meanSubtractedRawData = meanSubtractedRawData - median(meanSubtractedRawData,2); % subtract median across channels
        end
        filteredBufferedRawData = filtfilt(b1,a1,meanSubtractedRawData);
        % Kilosort does filtfilt like this:
