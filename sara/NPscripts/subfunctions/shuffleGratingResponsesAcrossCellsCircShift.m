...
function [data_shuf] = shuffleGratingResponsesAcrossCellsCircShift(data, ind)

    shiftIdx = randi([1,length(ind)]); % choose random integer in range of indices
    newIdx = circshift(ind,shiftIdx);
    data_shuf = [];
