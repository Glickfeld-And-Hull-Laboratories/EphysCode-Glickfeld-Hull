...
function [spikeCounts] = getSpkTimesForRFConvolution_Wiesel(cells,times,binsize,iexp)
    ...
    % Extract units from KS output
        load(fullfile(base, '/sara/Analysis/Neuropixel', [exptStruct.date], [exptStruct.date '_' exptStruct.mouse '_unitStructs.mat']), 'goodUnitStruct');
    ...
    % Initialize
    nCells = length(cells);
    timestamps_flat = timestamps(:);   % 9000×1 or 12000x1
    spikeCounts = zeros(length(timestamps_flat), nCells);  % 9000×nCells or 12000xnCells
