...
cd(fullfile(base, exptStruct.loc, 'analysis', 'neuropixel', exptStruct.mouse, exptStruct.date, 'kilosort4/')) % Navigate to KS_Output folder
...
%% Load timestamps and downsampled white noise stimulus
    ...
    % Load stim on information (both MWorks signal and photodiode)
        cd (fullfile(base, exptStruct.loc, 'analysis', 'neuropixel',  exptStruct.mouse, exptStruct.date))        % Move from KS_Output folder to ...\Analysis\neuropixel\date folder, where TPrime output is saved
        
        % Get all subdirectories
        d = dir();
        d = d([d.isdir]);   % keep directories only
        
        % Find directory whose name starts with 'catgt' and contains 'CenterSurround'
        idx = find(startsWith({d.name}, 'catgt') & contains({d.name}, 'CenterSurround'), 1);
        ...
        stimOnTimestampsMW  = table2array(readtable([mouse '_' date '_mworksStimOnSync.txt']));
        stimOnTimestampsPD  = table2array(readtable([mouse '_' date '_photodiodeSync.txt']));
    ...
    load(fullfile(base, 'sara', 'Analysis', 'Neuropixel', 'noiseStimuli/', '5min_2deg_4rep_imageMatrix.mat'))
