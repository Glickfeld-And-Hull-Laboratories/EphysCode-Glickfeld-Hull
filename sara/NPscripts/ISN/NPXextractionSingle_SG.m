%% 
% Jerry's NP_getData equivalent script for analyzing the V1 cross ori
% center surround data
%
%%

clear all; clc; clear global;

% 1 2 3, with RF
% 11 9 6 5, from lab mtg presentation
...
subplot 221
    ...
    print(fullfile(['\\duhs-user-nc1.dhe.duke.edu\dusom_glickfeldlab\All_Staff\home\sara\Analysis\Neuropixel\ISN_Jerry\' exptStruct.mouse '-' exptStruct.date '-FRbyDepth.pdf']),'-dpdf','-bestfit');
...
[trialStruct, gratingRespMatrix, gratingRespOFFMatrix, resp, base] = createTrialStruct_ISN_CentSurr(stimStruct, goodUnitStruct);     

[avg_resp_dir, resp_ind_dir] = getResponses_ISN_CentSurr(resp, base);
...
nDirs  = size(resp,2);
nCon   = size(resp,3);
time = (0:size(resp{1,1,1},2)-1) * binSize;
cellsToPlot = 21:1:40;
...
for ic = 1:nCells
    ...
    for id = 1:nDirs
        for icond = 1:nCon
            ...
            psth = mean(r,1,'omitnan');
            plot(time, psth, 'LineWidth', 1.5); hold on
            if ic ==1
                legendStr{end+1} = sprintf('C%d', icond);
            end
        end
    end
    xlabel('Time (s)');
    ylabel('Firing rate (Hz)');
    % title(sprintf('Cell %d PSTH', ic));
    box off;
    text(0.02, 0.9, num2str(depth_all(ic)),'Units','normalized','FontSize',8);
    if ic == 1
        legend(legendStr, 'Location','bestoutside');
        lgd.ItemTokenSize = [6 4];
        lgd.FontSize = 8;
    end
    s=s+1;
    if s > 20
        orient('landscape')
        print(fullfile(['\\duhs-user-nc1.dhe.duke.edu\dusom_glickfeldlab\All_Staff\home\sara\Analysis\Neuropixel\ISN_Jerry\' exptStruct.mouse '-' exptStruct.date '-PSTHs_' num2str(n) '.pdf']),'-dpdf','-bestfit');
        figure;
        s=1;
        n=n+1;
    end
...
stimBarY = mean(psth_data(:,:,1:20),"all");
