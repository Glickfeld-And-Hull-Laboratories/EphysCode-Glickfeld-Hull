...
date = '250314';
mouse = 'i2748';
mwtime = '1429';    % Right now, highConORiBar
chnls = 1:2:220;
...
% Load MW files
    ...
    load(bName);
    stimElevation   = cell2mat(input.tGratingElevationDeg);
...
% Check that PD signal starts at same time as MW signal; sometimes there are errant PD signals 
    ...
    timestamps = leadingEdgesPD(1:201)'; 
    fprintf('temporary indexing of timestamps for testing script----get rid of this!! \n')
...
% Get average LFP for each retinotopic location
   ...
   for is = 1:nStim
        subplot(nElevations,nAzimuths,is)
            ...
            xlabel('time')
            ylabel('channel')
            zlabel('amp (mV)'); zlim([-30 100])
        title(num2str(stims(is,:)))
   end

print(['\\duhs-user-nc1.dhe.duke.edu\dusom_glickfeldlab\All_Staff\home\sara\Analysis\Neuropixel\' date '\' date '_' mouse '_retinotopy.pdf'], '-dpdf','-bestfit')
